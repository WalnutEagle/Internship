...
class CarlaEnv(gym.Env):
    ...
    def action_space(self):
        ...
        return spaces.Box(low=-1.0,high=1.0,dtype=np.float64)
    ...
    def _get_obs(self):
        ...
        for i in self.pedestrian_list:
            ...
            ped_distances.append(self._get_actor_distance(i, player_pos))
            if self._get_actor_direction(i, player_pos)!=None:
                self.ped_count+=1
        ...
        return np.array([pedestrian_detected,collision_detected,distance_covered,distance_to_nearest_pedestrian], dtype=np.float64)
        #distance_covered,
    ...
    def reset(self,seed=None, options=None):
        ...
        if(self.count>1):
            ...
            loc_center = [player_location.location.x, player_location.location.y, player_location.location.z ]
            self.val1,self.s=util.spawn_walkers_dense(self.client, self.world.world, 1, loc_center,0, 0.9, 0.2, self.all_id_dict,self.count,self.s)
            if not self.val1:
                print("spawn_walkers_dense failed")
                return
