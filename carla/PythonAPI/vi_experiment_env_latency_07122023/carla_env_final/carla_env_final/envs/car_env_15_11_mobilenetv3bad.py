...
class CarlaSwitchV3badEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        # self.model=torch.load("/home/kathakoli/carla/PythonAPI/vi_experiment_env_latency_07122023/CVPR_mobilenet.pth")
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([0.0]*737), high=np.array([1.0]*737), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location, pedestrian_detected):
        ...
        if pedestrian_detected==0:
            speed_new = resnew[0][1]-random.uniform(0.6,0.8)
            rotation_new=resnew[0][0]
        else:
            speed_new = resnew[0][1]-random.uniform(0.9,1.1)
            rotation_new=resnew[0][0]
