...
def setup1():
    ...
    env["CARLA_ROOT"]="/data/carla/Unreal/CarlaUE4/Saved/StagedBuilds/LinuxNoEditor/"
...
def setup(
    ...
    argparser.add_argument(
        '--path',
        default="/data/carla/PythonAPI/vi_experiment_env_latency_07122023/demo_rh/path_points_t10_32_95.npy",
        type=str,
    ...
    s=[]
    if not util.spawn_walkers(client, world.world, 30, 0, 0.9, 0.2, sparse_id_dict):
    ...
    dest_points=np.array([minimap.planner.path_pts[-1][0],minimap.planner.path_pts[-1][1]])
    val1=util.spawn_walkers_dense(client, world.world, 30, loc_center,0, 0.9, 0.2, dense_id_dict,dest_points)
    if not val1:
    ...
    return client, world,server, minimap, state_manager, traffic_manager, controller, display,sparse_id_dict ,dense_id_dict, vehicles_list,s
...
def docker_setup(
    ...
    vehicles_list = util.spawn_vehicles(client, world.world, 20, True, traffic_manager)
    # pedestrian_seed = 42
    # world.world.set_pedestrians_seed(pedestrian_seed)  
    s=[]
    if not util.spawn_walkers(client, world.world, 30, 0, 0.9, 0.2, sparse_id_dict):
    ...
    # print("loc_center: ", loc_center)
    dest_points=np.array([minimap.planner.path_pts[-1][0],minimap.planner.path_pts[-1][1]])
    val1=util.spawn_walkers_dense(client, world.world, 70, loc_center,0, 0.9, 0.2, dense_id_dict,dest_points)
    if not val1:
        print("spawn_walkers_dense failed")
        return
    # cameras = spawn_cameras(world.world, world.player, world.destination_vehicle, args.width, args.height, 90)
