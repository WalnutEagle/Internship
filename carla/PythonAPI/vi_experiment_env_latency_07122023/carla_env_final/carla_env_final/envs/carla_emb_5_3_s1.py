...
class CarlaEnvrouters2(gym.Env):
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # high_value=[1.0]*2560
        return spaces.Box(low=np.array([-1.0]*737), high=np.array([1.0]*737), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            a1=np.random.normal(action_expert_angle, latency)
            a1_clipped = np.clip(a1, -np.pi/8, np.pi/8)
            a2=np.random.normal(action_expert_speed, latency)
            a2_clipped = np.clip( action_expert_speed, -1.5, 1.5)
            print(action_expert_angle)
    ...
    def close(self):
        ...
        self.client.apply_batch([carla.command.DestroƒlatenyActor(x)
                           for x in self.dense_id_dict['controllers']])
        self.client.apply_batch([carla.command.DestroyActor(x)
