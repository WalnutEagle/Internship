...
class CarlaEnv5(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.model=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth")
    ...
    def _get_obs(self):
        ...
        #     else:
        #         collision_detected = 0.0
        self.curr_pos_x,self.curr_pos_y=self.world.player.get_transform().location.x,self.world.player.get_transform().location.y
    ...
    def reset(self,seed=None, options=None):
        ...
        # self.world.tick(self.clock)
        # self.pos_x,self.pos_y=self.world.player.get_transform.location.x,self.world.player.get_transform.location.y
    
        #cv2.imwrite('image.jpg',self.eyelevel_rgb_array)
        #self.eyelevel_rgb_array.save_to_disk('_out/image.jpg')
        # print("Obs at reset:", obs)
        # print(type(obs))
        self.pos_x=self.world.player.get_transform().location.x
        self.pos_y=self.world.player.get_transform().location.y
    ...
    def get_reward(self,collision, action,geodesic):
        ...
        dist=np.linalg.norm(np.array([self.world.player.get_transform().location.x,self.world.player.get_transform().location.y])-np.array([self.pos_x, self.pos_y]))
        speed = action[0][1]-self.speed_x
        self.speed_x=action[0][1]
        self.pos_x=self.world.player.get_transform().location.x
        self.pos_y=self.world.player.get_transform().location.y
        return dist+(0.05*speed)- (0.00002*collision)
