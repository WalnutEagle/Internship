...
class CarlaSwitchSpeedEnv(gym.Env):
    ...
    def local_policy(self, image, pathclosex, pathclosey,pedestrian_detected):
        ...
        #self.action_list = np.vstack((self.action_list, np.array([angle,self.speed_new,0.0])))
        return self.action_list[-4:]
    ...
    def get_reward(self,collision, action,geodesic,dir_r, Flop):
        ...
        if collision!=0 and action[1]>1e-5:
            ...
            #print("Geodesic",geodesic_rw)
                #print(geodesic)
                #speed=(action[0][1])/2x
            power_dis=(1.0-math.tanh(Flop/19.3))#Flop/Max Flop
            speed=self.world.player.get_velocity().length()/2
            # print("Speed",speed)
            #return geodesic_rw
            print((geodesic_rw*speed*extreme_actions_reward*power_dis)**(1.0/4.0))
        #return (geodesic_rw*speed)**(1.0/2.0)
        return speed
