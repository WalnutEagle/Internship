...
class CarlaEval(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.expert_policy_list=np.array([[0.0, 0.0] for _ in range(4)])
        # self.all_policies=np.array([[0.0, 0.0,0.0] for _ in range(4)])
        self.model=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/CVPRMODEL207.pth")
        ...
        self.result = cv2.VideoWriter("thesis_vis_19.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("thesis_vis_minimap_19.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([[-3.14, 0.0] for _ in range(4)]), high=np.array([[3.14, 2.0] for _ in range(4)]),shape=(4,2), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def expert_policy(self,pedestrian_detected,image,location):
        img_tensor = self.transform2(self.normalize(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560]).unsqueeze(0))
        ...
        # print(self.speed_new,"Speed")
        # print(self.rotation_new)
        # # print(self.all_policies,"Hi")
        # # print(len(self.all_policies))
        # self.all_policies = np.vstack((self.all_policies, np.array([self.rotation_new, self.speed_new,self.sampled_latency,self.sampled_latency])))
        # for i in range(len(self.all_policies)):
        #     # print(self.all_policies[i],"Hello")
        #     # print(type(self.all_policies[i]))
        #     if self.all_policies[i][3]==0:
        #         self.expert_policy_list = np.vstack((self.expert_policy_list,self.all_policies[i][:3]))
        #         # self.expert_policy_list.append(self.all_policies[i][:3])
        #         self.all_policies[i][3]=np.inf
        #     else:
        #         self.all_policies[i][3]-=1
    ...
    def _get_obs(self):
        ...
        self.ped_count=self.col_count=0
        ...
        for i in self.pedestrian_list:
            ...
            ped_distances.append(self._get_actor_distance(i, player_pos))
            if self._get_actor_distance(i, player_pos)<2 and self._get_actor_direction(i, player_pos)!=None:
                self.ped_count+=1
        ...
        start = time.time_ns()
        policy_list=self.expert_policy(pedestrian_detected, self.eyelevel_rgb_array,self.waypt)
        self.inference_latency = (time.time_ns() - start)/(10**9)
        # for i in self.pedestrian_list:
        #     #print(i)
        #     #ped_distances.append(self._get_actor_distance(i, player_pos)
        ...
        # if len(collision_history.keys()) > 0 and all(value==0.0 for value in collision_history.values())==False:
        #     # print("Hey")
        ...
        #     else:
        #         collision_detected = 0.0
        # distance_covered=np.linalg.norm(np.array([self.minimap.player_pos.x,self.minimap.player_pos.y]) - np.array([self.prev_x,self.prev_y]))
        ...
        return policy_list,np.array([pedestrian_detected,collision_detected,distance_covered,distance_to_nearest_pedestrian], dtype=np.float64)
        #distance_covered,
    ...
    def reset(self,seed=None, options=None):
        ...
        if(self.count>1):
            if self.count > 11:
                ...
                self.expert_policy_list=np.array([[0.0, 0.0] for _ in range(4)])
                #t=len(self.all_policies)
                # self.all_policies=np.array([[0.0, 0.0,0.0,np.inf] for _ in range(4)])
                # print(self.all_policies,"Inside reset")
        ...
        return obs[0],self.info

    def get_reward(self,collision, action,geodesic):
        ...
        if collision!=0 and action[0][1]>1e-5:
            #print("Diya")
            return -10.0
        else:
            ...
            return (geodesic_rw*speed*extreme_actions_reward)**(1.0/3.0)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        # if self.speed_new<0.4:
        #     action[0][1]=0.0
        # else:
        #     action[0][1]=self.speed_new
        action[0][0]=self.rotation_new
        action[0][1]=self.speed_new
        x_dir = math.cos(action[0][0])
        y_dir = math.sin(action[0][0])
        print(action[0][0],self.frame_step)
        print(action[0][1],self.frame_step)
        self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
        self.controller._control.speed = action[0][1]
        self.world.player.apply_control(self.controller._control)
        obs = self._get_obs()
        self.l2_distance=obs[1][2]
        self.route_completion+=self.l2_distance
        self.distance_covered+=self.l2_distance
        reward=self.get_reward(obs[1][1],action,obs[1][2])
        # self.l2_distance=obs[1][2]
        # self.route_completion+=self.l2_distance
        # self.distance_covered+=self.l2_distance
        # reward=self.get_reward(obs[1][1],action,obs[1][2])
    ...
    #print(p)
        ...
        #print(p)
            ...
            self.task_metrics.append([self.destination_success, self.infraction_ped_collision,self.percentage_route_completion, self.route_deviation, self.route_length*self.percentage_route_completion/100000.0, episode_time])
            #print("Energy",self.energy,"mJ")
        ...
        return obs[0], reward, done, truncated,self.info
    ...
    def close(self):
        ...
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Route Completion (%)", "Avg. Route Deviation (m)", "Distance covered (km)", "Time taken (s)"])
        df.to_csv('thesis_07_11_taskmetrics',index=False)
        self.client.apply_batch([carla.command.DestroyActor(x)
