...
class CarlaEnvembs2(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.l=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/Local_model_regnet_14_11.pth")
        self.l=self.l.to(torch.device('cuda'))
        #print("Sleep")
        self.model=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/CVPRmodelvit.pth")
        ...
        # self.result = cv2.VideoWriter("video_final_supp_6.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        # self.minimap_video = cv2.VideoWriter("final_minimap_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # high_value=[1.0]*2560
        return spaces.Box(low=np.array([-np.inf]*737), high=np.array([np.inf]*737), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location, pedestrian_detected):
        ...
        #an = self.l.lin(res1)
        img_tensornew = self.normalize(self.transform2(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560])).unsqueeze(0)
        location_tensornew = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
        resnew=self.model(img_tensornew.to(torch.device('cuda')), location_tensornew.to(torch.device('cuda')))
        resnew=resnew.cpu().detach().numpy()
        ...
        # print(resnew,"resnew")
        return resnew[0][0], float(resnew[0][1]),res1

    def expert_policy(self,pedestrian_detected,image,location,res1):
        #print("Hello")
        #self.q+=1
    ...
    def _get_obs(self):
        ...
        for i in self.pedestrian_list:
            ...
            #ped_distances.append(self._get_actor_distance(i, player_pos))
            ped_distances.append(self._get_actor_distance(i, player_pos))
            if self._get_actor_distance(i, player_pos)<3 and self._get_actor_direction(i, player_pos)!=None:
                self.ped_count+=1
        #print(sorted(ped_distances))
        # print(np.sort(col_count))
        self.pedestrian_detected = 1.0 if self.ped_count > 1 else 0.0
    ...
    def step(self, action):
        ...
        if action==1:
            ...
            while self.lat_count>0:
                ...
                self.lat_count-=1
                total_reward+=rew
            if self.la>self.lat_count:
                total_reward/=(self.la-self.lat_count)
            if self.col_count>1:
                total_reward-=30.0
        else:
            obs, rew, done,truncated, info= self._step(action)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            if self.lat_count>0:
                ...
                action_expert_angle,action_expert_speed=self.expert_policy(self.pedestrian_detected,self.eyelevel_rgb_array,self.waypt,res1)
                # an=action_expert[0]
                
                # if self.pedestrian_detected==1.0 or action_expert_angle>0.5:
                #     mode=random.choice(['Local','Cloud'])
                #     if mode=='Cloud':
                #         energy=1.5
                #         latency_n=random.uniform(0,0.2)
                #     else:
                #         energy=0.15
                #         latency=0.0
                # else:
                ...
                #     energy=0.15
                # if mode=='Cloud':
                # latency=random.uniform(0,0.05)
                # # latency=0.0
                # a1=np.random.normal(action_expert_angle, latency)
                # # a1_clipped = np.clip(a1, -np.pi/4, np.pi/4)
                # a2=np.random.normal(action_expert_speed, latency)
                # a2_clipped = np.clip(a2, -1.5, 1.5)
                # print("Hey")
                # print(a1)
                # print(a2)
                # x_dir = math.cos(a1)
                # y_dir = math.sin(a1)
                # self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
                # self.controller._control.speed = a2_clipped
                # else:
                # latency=random.uniform(0,0.2)
                # a1=np.random.normal(action_expert_angle, latency)
                a1= np.clip(action_expert_angle, -np.pi/8, np.pi/8)
                # a2=np.random.normal(action_expert_speed, latency)
                a2 = np.clip( action_expert_speed, -1.5, 1.5)
                # print(action_expert_angle,action_expert_speed,"Cloud Action")
                # print(action_expert_speed)
                x_dir = math.cos(action_expert_angle)
                y_dir = math.sin(action_expert_angle)
                self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
                self.controller._control.speed =a2
                energy=1.65
            print("Inside Cloud")
        ...
        print(self.controller._control.direction,self.controller._control.speed, self.lat_count, mode)
        ...
        # if self.infraction_ped_collision>1:
        #     self.infraction_ped_collision=1
        # mode_loc = (5, 15)
        # infraction_loc = (5,40)
        # energy_loc = (5,65)
        # latency_loc = (5, 90)
    ...
    #         truncated = True
        ...
        #print(p)
            ...
            self.task_metrics.append([self.count_epi,self.infraction_ped_collision,self.route_completion])
    ...
    def close(self):
        ...
        # df.to_csv('task_metrics_ablation_final.csv',index=False)
