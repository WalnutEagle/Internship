...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        self.client, self.world, self.server, self.minimap, self.state_manager, self.traffic_manager,self.controller,self.display,self.cameras=setup()
        self.collision_hist = []
        #self.actor_list = []
        self.frame_step = 0
        #self.out_of_loop = 0
    ...
    def reset(self):
        ...
        self.world.restart()
        # vehicles_list = util.spawn_vehicles(self.client, self.world.world, 200,  True, self.traffic_manager)


        # if not util.spawn_walkers(self.client, self.world.world, 50, 0.01, 0.9, 0.2, self.all_id_dict):
        ...
        # if not util.spawn_walkers_dense(self.client, self.world.world, dense_N, loc_center,dense_percent_disabled,0.9,0.2, self.all_id_dict):
        ...
        # self.cameras = spawn_cameras(self.world.world, self.world.player, self.world.destination_vehicle,
        #                         640, 360, 90)
        print("Hello")
        ...
        im = Image.fromarray(self.eyelevel_rgb_array)
    ...
    def step(self, action):
        ...
        for _ in range(self.repeat_action):
            obs, rew, done= self._step(action)
            total_reward += rew
            if done:
                break
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        #     image = image * 255
        print(self.eyelevel_rgb_array.shape)
        im1 = Image.fromarray(self.eyelevel_rgb_array)
    ...
    def _get_num_people(self,image, model):
        result = self.model(self.image).xyxy[0]
