...
class CarlaSwitchEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0.0]*2561 for _ in range(4)])
        ...
        self.l=torch.load("local_model_mobilenetv2_100_14_11.pth")
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # high_value=[1.0]*2560
        return spaces.Box(low=np.array([[0.0]*2561 for _ in range(4)]), high=np.array([[1.0]*2561 for _ in range(4)]), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location):
        ...
        img_tensor = self.normalize(self.transform2(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560])).unsqueeze(0).to(torch.device('cuda'))
    ...
    def get_reward(self,collision, action0,action1,geodesic,dir_r, energy):
        ...
        if collision!=0 and action1>1e-5:
            ...
            #print("Geodesic",geodesic_rw)
                #print(geodesic)
                #speed=(action[0][1])/2x
            energy_dis=(1.0-math.tanh(energy/2.5))#Flop/Max Flop
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            x_dir = math.cos(float(a1))
            y_dir = math.sin(float(a1))
            self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
            self.controller._control.speed = float(a2)
            energy=0.15
            print("Inside Local")
        elif action==1:
            a1,a2=self.expert_policy(self.pedestrian_detected,self.eyelevel_rgb_array,self.waypt)
            #an=action_expert[0]
        ...
        reward=self.get_reward(obs[0],float(a1),float(a2),obs[1],obs[2],energy)
