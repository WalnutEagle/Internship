...
class CarlaEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        cv2.waitKey(1)
        resized_image = self.eyelevel_rgb[:,0:576,224:800]
        ...
        image1=self.transform(final_data)
        image= self.transform2(image1)
        model=torch.load('model_affordance.pth')
        affordance=model.forward(image.unsqueeze(0))
        #self.sync_mode.tick(None)
        ...
        policy_final=np.concatenate((policy_list,affordance))
        return policy_final,np.array([pedestrian_detected,collision_detected,distance_covered,distance_to_nearest_pedestrian], dtype=np.float64)
        #distance_covered,
