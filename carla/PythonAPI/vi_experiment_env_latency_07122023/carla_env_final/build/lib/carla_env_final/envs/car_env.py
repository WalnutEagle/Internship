...
class CarlaEnv(gym.Env):
    ...
    #     return seed
    def local_policy(self, image, pathclosex, pathclosey):
    ...
    def expert_policy(self,pedestrian_detected):
        ...
        if pedestrian_detected==0:
            ...
            # self.q=0 
        #print("Rotation",self.rotation_new)
    ...
    def get_reward(self,collision, action,geodesic,dir_r, Flop):
        ...
        if collision!=0 and self.world.player.get_velocity().length()>1e-5:
            ...
            return (geodesic_rw*speed*dir_r*power_dis)**(1.0/4.0)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0.0:
            ...
            #an=action_expert[0]
            x_dir = math.cos(action_expert[0])
            y_dir = math.sin(action_expert[0])
            self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
            self.controller._control.speed = action_expert[1]
            print("Inside Cloud")
        ...
        reward=self.get_reward(obs[0],action,obs[1],obs[2],Flop)
