...
sys.path.append('/data/carla/PythonAPI/vi_experiment_env_latency_07122023/carla_env_final/carla_env_final/envs/')
...
class CarlaEnvCVPR04(gym.Env):

    metadata = {'render_modes': ['human']}
    def __init__(self, args,vehicles_list,all_id_dict):
        #self.server=setup1()
        self.client, self.world, self.server,self.minimap, self.state_manager, self.traffic_manager,self.controller,self.display, self.sparse_id_dict, self.dense_id_dict, self.vehicles_list,self.s=docker_setup()
        ...
        # self.model_yolo = torch.hub.load("ultralytics/yolov5","yolov5s",trust_repo=True,device=1)
        # self.all_id_dict=all_id_dict
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0.0, 0.0,0.0] for _ in range(4)])
        self.sampled_latency=0
        self.q=0
        self.ct=0
        self.prev_angle=0.0
        self.global_model = torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth").module.to(torch.device('cuda'))
        # self.model=torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_train_final_local_07_11.pth").module.to(torch.device('cuda'))
        # self.l=ClassificationNetwork()
        # self.l.load_state_dict(torch.load("local_model_10_11_mobilenet.pth").module.state_dict())
        # self.l=torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/Local_model_regnet_14_11.pth")
        # self.l=self.l.to(torch.device('cuda'))
        self.model=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/model_train_final_local_07_11.pth").module
        ...
        self.localdir=carla.Vector3D(0.0,0.0,0.0)
        self.localspe=0.0
        self.clouddir=carla.Vector3D(0.0,0.0,0.0)
        self.cloudspe=0.0
        # self.a=torch.Tensor([0.0]*2560)
        # self.infraction_ped_collision = 0
        # self.prev_collision = []
        # self.curr_collision = []
        # self.episode_start_time = time.time()
        # self.cloud_policy_time = 0.0

        # Route Completion calculation
        # print(self.minimap.planner.path_pts[-1,0:1], self.minimap.planner.path_pts[-1,1:2])
        # print(type(self.minimap.planner.path_pts[-1,0:1]), type(self.minimap.planner.path_pts[-1,1:2]))
        # self.target_wp = carla.Location(x=self.minimap.planner.path_pts[-1,0:1][0],y=self.minimap.planner.path_pts[-1,1:2][0],z=0.0)
        # self.current_wp_index = 0
        # pos = [self.world.player.get_location().x,self.world.player.get_location().y]
        # self.start_waypt_index = self.minimap.planner.get_closest_point(pos)
        # self.start_waypt = self.minimap.planner.path_pts[self.start_waypt_index,0:2]

        # pos = [self.world.destination_vehicle.get_location().x,self.world.destination_vehicle.get_location().y]
        # self.dest_waypt_index = self.minimap.planner.get_closest_point(pos)
        # self.dest_waypt = self.minimap.planner.path_pts[self.dest_waypt_index, 0:2]

        # self.route_length = self.minimap.planner.get_path_length_from_position(self.start_waypt, self.dest_waypt)
        # # self.start_waypt_index = np.where((self.minimap.planner.path_pts==self.start_waypt).all(1))[0][0]
        # self.waypts = self.minimap.planner.path_pts[self.start_waypt_index:]
        # self._accum_meters = self.minimap.planner.path_pts[self.start_waypt_index:,2] - self.minimap.planner.path_pts[self.start_waypt_index,2]
        # print(self._accum_meters)


        # self.result = cv2.VideoWriter("new_experiments/rollout_ablation_CVPR_04_dense_path_32_new.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        # self.minimap_video = cv2.VideoWriter("new_experiments/minimap_CVPR_04_dense_path_32_new.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([[-3.14, 0.0,0.0] for _ in range(4)]), high=np.array([[3.14, 2.0,1.0] for _ in range(4)]),shape=(4,3), dtype=np.float64)
        #0.0,, np.inf 
    ...
    #     return seed
    def local_policy(self, image, location):
        ...
        img_tensor = self.normalize(self.transform2(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)))).unsqueeze(0)
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
        res=self.model(img_tensor.to(torch.device('cuda')), location_tensor.to(torch.device('cuda')))
        res=res.cpu().detach().numpy()
        self.action_list = np.vstack((self.action_list, np.array([res[0][0],res[0][1],0.0])))
        energy=0.15
        return self.action_list[-4:],energy
    ...
    def expert_policy(self,pedestrian_detected,image,location):
        ...
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
        res=self.global_model(img_tensor.to(torch.device('cuda')), location_tensor.to(torch.device('cuda')))
        res=res.cpu().detach().numpy()
        if pedestrian_detected==0:
        ...
        #     self.speed_new = 0
        #     self.rotation_new=0
        # img_tensor = self.normalize(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))).unsqueeze(0).to(torch.device('cuda'))
    ...
    def pre_act(self):
        ...
        waypt=np.array([path_next_x,path_next_y]) - np.array([self.world.player.get_transform().location.x,self.world.player.get_transform().location.y])
        a,energy=self.local_policy(self.eyelevel_rgb_array,waypt)
        
        return a,pedestrian_detected,energy,waypt
    ...
    def _get_obs(self):
        ...
        # self.a=self.local_policy(self.eyelevel_rgb_array,self.waypt)
        ...
        # # # policy_list=self.expert_policy(pedestrian_detected)
        # # self.cloud_policy_time += (time.time()-start_time)
        # self.curr_collision = []
        # for i in self.pedestrian_list:
        #     #print(i)
        #     #ped_distances.append(self._get_actor_distance(i, player_pos)
        ...
        #         #print(self._get_actor_distance(i, player_pos),'Hi')
        #         self.col_count+=1
        #         if i.id != self.world.player.id:
        #             self.curr_collision.append(i.id)
        # collision_detected = 0.0
        # if self.col_count > 1 and (sorted(self.curr_collision) != self.prev_collision):
        #     collision_detected = 1.0
        
        # self.infraction_ped_collision+= int(collision_detected)
        # self.prev_collision = sorted(self.curr_collision)
        ...
        # path_next_x,path_next_y=self.minimap.planner.get_next_goal(pos=[pathxclose,pathyclose],preview_s=2)
        # #print(path_next_x,path_next_y)
        # distance_covered=np.linalg.norm(np.array([pathxclose,pathyclose])-np.array([self.minimap.player_pos.x,self.minimap.player_pos.y]))
        # ped_vector=carla.Location(pathx,pathy,0)-carla.Location(self.minimap.player_pos.x,self.minimap.player_pos.y,0)
        # #print(ped_vector)
    ...
    def get_reward(self,a,collision, action,geodesic,dir_r, energy):
        ...
        if collision!=0 and action[1]>1e-5:
            ...
            return -30.0
            # else:
            #     return -50.0*(self.l-self.lat_count)
        else:
            extreme_action0_rw = 1.0 if abs(action[0]/3.14) < 0.97 else 0.0
            extreme_action1_rw = 1.0 if abs(action[1]/2)< 0.97 else 0.0
            ...
            #print(geodesic_rw)
            #speed=(action[0][1])/2x
            energy_dis=(1.0-energy/2.25)#Flop/Max Flop
    ...
    def step(self, action):
        ...
        if action==1:
            self.lat_count=random.choice(range(1,10))
            self.l=self.lat_count
            while self.lat_count>0:
                # print("Inside cloud waiting")
                obs, rew, done,truncated, info= self._step(action)
                if truncated or done:
                    break
                self.lat_count-=1
                total_reward+=rew
            if self.l>self.lat_count:
                total_reward/=(self.l-self.lat_count)
            if self.col_count>1:
                total_reward-=30.0
        else:
            obs, rew, done,truncated, info= self._step(action)
            total_reward += rew
        print(total_reward, "Total Reward")
        return obs, total_reward, done,truncated,info

    # Steps environment
    def _step(self, action):#0- Local Policy #1- Global Policy
        self.l2_distance=0
        # self.destination_success = 0
        self.world.tick(self.clock)
        self.render()
        #print(action)
        self.frame_step += 1
        ...
        #obs = self._get_obs()
        # start = time.time_ns()
        a, pedestrian_detected,energy,waypt=self.pre_act()
        # inference_latency = (time.time_ns() - start)/(10**9)
        ...
        #print(action)
        # a1,a2,res1=self.local_policy(self.eyelevel_rgb_array,self.waypt,self.pedestrian_detected)
        # print(self.lat_count,"Lat Count")
        # if self.lat_count>0:
        #     mode="Cloud waiting local action"
        #     self.controller._control.direction = self.localdir
        #     self.controller._control.speed = self.localspe
        #     self.lat_count-=1
        #     self.action_list[-1][2]=1.0
        #     energy+=self.e
        # elif self.lat_count==0:
        #     mode = "Cloud reached action"
        #     self.controller._control.direction = self.clouddir
        #     self.controller._control.speed = self.cloudspe
        #     self.lat_count-=1
        #     self.action_list[-1][2]=1.0
        #     energy+=self.e
        if action==0:
            ...
            # an=self.action_list[-1][0]
            x_dir = math.cos(an)
            y_dir = math.sin(an)
            # self.localdir=carla.Vector3D(math.cos(self.action_list[-1][0]),math.sin(self.action_list[-1][0],0.0)
            # self.localspe=self.action_list[-1][1]
            self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
            self.controller._control.speed = a[-1][1]
            ...
            # start = time.time_ns()
            if self.lat_count>0:
                self.controller._control.direction=carla.Vector3D(math.cos(self.action_list[-1][0]),math.sin(self.action_list[-1][0]),0.0)
                self.controller._control.speed = self.action_list[-1][1]
                self.action_list[-1][2]=1.0
                energy+=1.5
            else:
                action_expert,e=self.expert_policy(pedestrian_detected,self.eyelevel_rgb_array,waypt)
                # inference_latency += (time.time_ns() - start)/(10**9)
                #an=action_expert[0]
                # latency=random.uniform(0,0.2)
                # latent_dir=np.random.normal(self.action_list[-1][0], latency)
                # latent_speed=np.random.normal(self.action_list[-1][1], latency)
                # x_dir = math.cos(self.action_list[-1][0])
                # y_dir = math.sin(self.action_list[-1][0])
                # self.clouddir=carla.Vector3D(x_dir,y_dir,0.0)
                # self.cloudspe=self.action_list[-1][1]
                # self.controller._control.direction = self.clouddir
                # self.controller._control.speed = self.cloudspe
                self.controller._control.direction=carla.Vector3D(math.cos(self.action_list[-1][0]),math.sin(self.action_list[-1][0]),0.0)
                self.controller._control.speed = self.action_list[-1][1]
                ...
                energy+=e
            print("Inside Cloud waiting",self.lat_count)
        ...
        # print(mode,"Mode")
        # print(energy,"Energy")
        # print(self.controller._control.direction, "Direction")
        # print(self.controller._control.speed, "Speed")
        ...
        self.distance_covered+=self.l2_distance # Fix
        reward=self.get_reward(action,obs[0],self.action_list[-1],obs[1],obs[2],energy)
        ...
        # # Add Collision Count
        # updated_img = cv2.putText(updated_img, f'Collision Count:', infraction_loc, font,
        #                             fontScale, color, thickness, cv2.LINE_AA)
        # updated_img = cv2.putText(updated_img, f'{self.infraction_ped_collision:02.0f}', (155,40), font,
        #                             fontScale, color, thickness, cv2.LINE_AA)
        ...
        if self.action_list[-1][1]>1e-5:
            if obs[0] != 0.0:
                ...
                # pass
                # truncated=True
        # Compute percentage route completion (Same formula as CARLA)
    #     for index in range(self.current_wp_index, min(self.current_wp_index+2+1,len(self.waypts))):
    #         # Get the dot product to know if it has passed this location
    #         waypoint_coords = self.waypts[index]
    #         waypoint = self.world.map.get_waypoint(carla.Location(waypoint_coords[0],waypoint_coords[1],0.0), project_to_road=False, lane_type=carla.LaneType.Any)
    #         if waypoint:
    #             wpt_dir = waypoint.transform.get_forward_vector()
    #             wp_ego_veh = self.world.player.get_transform().location - carla.Location(waypoint_coords[0],waypoint_coords[1],0.0)
    ...
    #     p=self.minimap.planner.get_closest_point(np.array([player_pos.x,player_pos.y]))
    # #print(p)
    #     pathxclose,pathyclose=self.minimap.planner.path_pts[p,:2]
    #     # local_goal = self.minimap.planner.get_next_goal(
    #     #     pos=[player_pos.x, player_pos.y], preview_s=5) # Change to closest point
    #     # local_goal = np.array(local_goal)[None, :]
    #     local_goal = np.array([pathxclose,pathyclose])
    
    #     ego_world_pos = np.array([[player_pos.x, player_pos.y]])
    #     self.route_deviation = np.linalg.norm(ego_world_pos - local_goal)
    #     if self.route_deviation >=3.0:
    #         truncated = True
        if self.state_manager.end_state:
            # end experiment
            # print("End state:",self.log['event'][-1])
            # self.percentage_route_completion = 100.0
            # self.destination_success = 1
            truncated = True
        ...
        #     ego_world_pos = np.array([[player_pos.x, player_pos.y]])
        #     self.route_deviation = np.linalg.norm(ego_world_pos - local_goal)
            # col_counts = self.world.collision_sensor.get_collision_ids()
            ...
            # self.local_energy_consumption = 10 * (episode_time-self.cloud_policy_time)
            #print("Energy",self.energy,"mJ")
            self.task_metrics.append([self.count_epi,self.col_count,self.route_completion])
            # self.task_metrics.append([self.destination_success,self.infraction_ped_collision,self.energy_consumption,self.total_latency,self.percentage_route_completion, self.route_deviation, episode_time, self.local_step_counter, self.global_step_counter])
            # self.episode_start_time = time.time()
            #print("Energy",)
    ...
    def close(self):
        ...
        # self.destination_success,self.infraction_ped_collision,self.local_energy_consumption,self.distance_covered, self.route_deviation
        # self.result.release()
        # self.minimap_video.release()
        # df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Energy Consumption (J)", "Total Latency (s)","Route Completion (%)", "Avg. Route Deviation (m)", "Time taken (s)", "Number of steps taken by local model", "Number of steps taken by global model"])
        # df.to_csv('new_experiments/task_metrics_ablation_CVPR_04_dense_path_32_new.csv',index=False)
