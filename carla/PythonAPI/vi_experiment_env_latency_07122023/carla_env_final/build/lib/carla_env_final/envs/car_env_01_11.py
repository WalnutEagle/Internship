...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.ct=0
        self.prev_angle=0.0
        l=os.listdir('collection_data_20_11')
        #l=sorted(l)
        find=[]
        if len(l)==0:
            new_dir='wk_1'
            ...
            for i in range(len(l)):
                find.append(int(l[i].split('_')[-1]))
            find=sorted(find)
            print(find)
            new_dir='wk_'+str(find[-1]+1)
            #print(l[-1])
        print(new_dir)
        
        if not os.path.exists(str('collection_data_20_11/')+str(new_dir)):
            os.makedirs(str('collection_data_20_11/')+str(new_dir))
        self.wk_path =str('collection_data_20_11/')+str(new_dir)
    ...
    def _get_obs(self):
        ...
        #     else:
        #         collision_detected = 0.0
        p=self.minimap.planner.get_closest_point(np.array([self.minimap.player_pos.x,self.minimap.player_pos.y]))
        #print(p)
        pathxclose,pathyclose=self.minimap.planner.path_pts[p,:2]
        pathx,pathy=self.minimap.planner.get_next_goal(pos=[self.minimap.player_pos.x,self.minimap.player_pos.y],preview_s=2)
        ...
        return policy_list,np.array([pedestrian_detected,collision_detected,distance_covered,r,distance_to_nearest_pedestrian], dtype=np.float64)
        #distance_covered,
    ...
    def reset(self,seed=None, options=None):
        ...
        if self.ct%10==0:
            ...
            pathxnew,pathynew=self.minimap.planner.get_next_goal(pos=[self.world.player.get_transform().location.x,self.world.player.get_transform().location.y],preview_s=5)
            np.save(os.path.join(self.wk_path, ('%012d_front.npy' % self.ct)), np.array([action[0],action[1],pathxnew,pathynew,self.world.player.get_transform().location.x,self.world.player.get_transform().location.y],dtype=np.float32))
        self.ct+=1
        #print(obs[0])
    ...
    def get_reward(self,collision, action,geodesic,dir_r):
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        obs = self._get_obs()
        action=obs[0][-1][:2]
        x_dir = math.cos(action[0])
        y_dir = math.sin(action[0])
        ...
        reward=self.get_reward(obs[1][1],action,obs[1][2],obs[1][3])
        ...
        if self.ct%10==0:
            ...
            # closepathx,closepathy=self.minimap.planner.path_pts[pa,:2]
            pathxnew,pathynew=self.minimap.planner.get_next_goal(pos=[self.world.player.get_transform().location.x,self.world.player.get_transform().location.y],preview_s=5)
            np.save(os.path.join(self.wk_path, ('%012d_front.npy' % self.ct)), np.array([action[0],action[1],obs[1][0],pathxnew,pathynew,self.world.player.get_transform().location.x,self.world.player.get_transform().location.y],dtype=np.float32))
        self.ct+=1
