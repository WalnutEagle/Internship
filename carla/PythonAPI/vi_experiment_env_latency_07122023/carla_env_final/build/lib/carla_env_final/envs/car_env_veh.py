...
from carla_env_final.envs.CarlaEnvSetupveh import setup,docker_setup
...
from local_model_veh_1 import Classification_Veh_Network_1
from local_model_veh_2 import Classification_Veh_Network_2
from cloud_model_veh import CloudVeh
import pandas as pd
...
# from run_experiment_xworld_rh_veh import HUD, World, MiniMap, StateManager, spawn_cameras, KeyboardControl
...
class CarlaEnvveh(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0.0, 0.0,0.0,0.0] for _ in range(4)])
        self.sampled_latency=0
        self.q=0
        self.ct=0
        self.prev_angle=0.0
        #self.global_model = torch.load("/home/kathakoli/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth")
        # self.infer_action= ClassificationNetwork()
        self.global_model = torch.load("modelcomp1.pth")
        self.m=torch.load("model_cloud_veh.pth")
        self.gm=torch.load("local_model_veh_1.pth")
        #print(self.global_model,"Helloooooo")
        # self.model=torch.load("")
        # self.checkpoint = torch.load('/data/carla/PythonAPI/vi_experiment_env_latency_07122023/local2/checkpoint_epoch_123.pth.tar', map_location='cuda')
        # self.m=Classification_Veh_Network_2()
        # if 'state_dict' in self.checkpoint:
        #     self.m.load_state_dict(self.checkpoint['state_dict'])
        # else:
        #     self.m.load_state_dict(self.checkpoint)
        # self.checkpoint_g = torch.load('/data/carla/PythonAPI/vi_experiment_env_latency_07122023/local1/checkpoint_epoch_123.pth.tar', map_location='cuda')
        # self.mg=Classification_Veh_Network_1()
        # if 'state_dict' in self.checkpoint_g:
        #     self.mg.load_state_dict(self.checkpoint_g['state_dict'])
        # else:
        #     self.mg.load_state_dict(self.checkpoint_g)
        self.transform = transforms.Compose([
            transforms.ToTensor(),
            ])
        self.transform3= transforms.Resize(96)
        ...
        self.result = cv2.VideoWriter("veh_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("veh_2_minimap.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([[-np.inf, -np.inf,-np.inf,0.0] for _ in range(4)]), high=np.array([[np.inf, np.inf,np.inf, 1.0] for _ in range(4)]), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location,pedestrian_detected):
        ...
        # print(image.shape)
        img_tensor = self.transform3(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,120:600:,400:880]).unsqueeze(0)
        ...
        # res=self.model.forward(img_tensor,location_tensor)
        self.m.eval()
        with torch.no_grad():
            res=self.m(img_tensor.to(torch.device('cuda')),location_tensor.to(torch.device('cuda')))
        res=res.cpu().detach().numpy()
        print(res,'Local')
        # res_final=self.scores_to_action(res)
        self.action_list = np.vstack((self.action_list, np.array([res[0][0],res[0][1], res[0][2],0.0])))
        energy=0.15
    ...
    def expert_policy(self,pedestrian_detected,image,location):
        ...
        # res=res.cpu().detach().numpy()
        img_tensor = self.transform3(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,120:600:,400:880]).unsqueeze(0)
        # im=cv2.resize(self.eyelevel_rgb_array,(640,360))
        # print(im.shape)
        # img_tensor_1=self.transform(cv2.cvtColor(im, cv2.COLOR_BGR2RGB)).unsqueeze(0)
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
        #res=self.global_model.forward(img_tensor)
        self.gm.eval()
        with torch.no_grad():
            res=self.gm(img_tensor.to(torch.device('cuda')),location_tensor.to(torch.device('cuda')))
        res=res.cpu().detach().numpy()
        print(res,'Cloud')
        # res_1=self.global_model.forward(img_tensor_1)
        # res_final=self.scores_to_action(res_1)
        # print(res_final,"Cloud2")
        # if pedestrian_detected==0:
        ...
        self.action_list[-1][0] =res[0][0]
        self.action_list[-1][1]=res[0][1]
        self.action_list[-1][2]=res[0][2]
        self.action_list[-1][3]=1.0
    ...
    def pre_act(self):
        ...
        a,energy=self.local_policy(self.eyelevel_rgb_array,waypt,pedestrian_detected)
    ...
    def _get_obs(self):
        ...
        #self.ct+=1
        # print(self.eyelevel_rgb_array.shape)
        ...
        # print(collision_history.values())
        # print(collision_history.keys())
        # if len(collision_history.keys()) > 0:
        #     if any("walker" or "vehicle" in collision_history.keys()):
        #         collision_detected = 1.0
        #     else:
        #         collision_detected = 0.0
        ...
        #     #print(i)
        #     # ped_distances.append(self._get_actor_distance(i, player_pos)
        #     print(self._get_actor_distance(i, player_pos),'dis')
        #     print(self._get_actor_direction(i, player_pos),'dir')
    ...
    def reset(self,seed=None, options=None):
        ...
        if(self.count>1):
            ...
            self.vehicles_list = util.spawn_vehicles(self.client, self.world.world, 20, True, self.traffic_manager)
            # batch = []
            # controller_list = []
            # controller_bp = self.world.world.get_blueprint_library().find('controller.ai.walker')
        ...
        # print(self.world.world.get_actors(),'jdfberjbfieb')
        self.pedestrian_list=[]
        self.p_list=self.world.world.get_actors().filter("walker.*")
        self.v_list=self.world.world.get_actors().filter("vehicle.*")
        self.pedestrian_list=list(self.p_list)+list(self.v_list)
        #self.pre_act()
        # print(self.pedestrian_list,'Diya')
        obs= self._get_obs()
        # action=obs[0][-1][:2]
        # if self.ct%10==0:
        #     pa=self.minimap.planner.get_closest_point(np.array([self.world.player.get_location().x,self.world.player.get_location().y]))
    ...
    def get_reward(self,collision, action,geodesic,dir_r, energy):
        ...
        if collision!=0 and action[2]<1e-5:
            print("Diyaaaaa")
            return -50.0
        else:
            extreme_action0_rw = 1.0 if abs(action[0]) < 0.97 else 0.0
            extreme_action1_rw = 1.0 if abs(action[1]/3.14)< 0.97 else 0.0
            #extreme_action2_rw = 1.0 if abs(action[2]/2)< 0.97 else 0.0
            ...
            print("Geodesic",geodesic_rw)
                #print(geodesic)
                #speed=(action[0][1])/2x
            energy_dis=(1.0-energy/4.25)#Flop/Max Flop
            print("Energy Reward",energy_dis)
            speed=self.world.player.get_velocity().length()/8
            print("Speed",speed)
            #print("Diya")
            #return geodesic_rw
        return (energy_dis*speed*extreme_actions_reward*geodesic_rw)**(1.0/4.0)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            mode = "Local"
            self.controller._control.throttle=0.8
            self.controller._control.steer=a[-1][1]
            if a[-1][2]<1.0:
                self.controller._control.brake=0.0
            else:
                self.controller._control.brake=1.0
            print("Inside Local")
        elif action==1:
            mode = "Cloud"
            ...
            self.controller._control.throttle=0.8
            latent_steer=np.random.normal(self.action_list[-1][1], latency)
            latent_brake=np.random.normal(self.action_list[-1][2], latency)
            self.controller._control.steer=latent_steer
            if latent_brake<1.0:
                self.controller._control.brake=0.0
            else:
                self.controller._control.brake=1.0
        ...
        self.l2_distance=obs[1]
        self.distance_covered+=self.l2_distance
    ...
    #print(p)
        ...
        #print(p)
            ...
            self.task_metrics.append([self.destination_success, self.infraction_ped_collision,self.energy_consumption,self.total_latency,self.percentage_route_completion, self.route_deviation,self.route_length*self.percentage_route_completion/100000.0, episode_time, self.local_step_counter, self.global_step_counter])
            # print("Env lasts {} steps, restarting ... ".format(self.frame_step))
            # print("Task Metrics:")
            # # print("Collision Count:",self.col_count)
            # print("Route Completion",(self.route_completion))
            # #print("Energy",self.energy,"mJ")
            # self.task_metrics.append([self.count_epi,self.route_completion])
            #print("Energy",)
    ...
    def close(self):
        ...
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Energy Consumption (J)", "Total Latency (s)","Route Completion (%)", "Avg. Route Deviation (m)", "Distance covered (km)", "Time taken (s)", "Number of steps taken by local model", "Number of steps taken by global model"])
        df.to_csv('veh_2.csv',index=False)
        self.client.apply_batch([carla.command.DestroyActor(x)
                           for x in self.vehicles_list])
