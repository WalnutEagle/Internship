...
class CarlaEnvrouters1(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.result = cv2.VideoWriter("rollout_ablation_DQN_baseline_high.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("minimap_DQN_baseline_high.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location,pedestrian_detected):
        ...
        emb=self.model(img_tensor.to(torch.device('cuda:1')), location_tensor.to(torch.device('cuda:1')), True).squeeze(0)
    ...
    def reset(self,seed=None, options=None):
        ...
        # self.world.tick(self.clock)
        self.a, emb,self.pedestrian_detected,self.energy,self.waypt=self.pre_act()
        ...
        # print(self.action_list[-1].shape)
        self.state=np.concatenate([np.array(emb), self.action_list[-1]], axis=0)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            self.controller._control.speed = self.a[-1][1]
            print("Inside Local")
        elif action==1:
            action_expert,e=self.expert_policy(self.pedestrian_detected,self.eyelevel_rgb_array,self.waypt)
            #an=action_expert[0]
        ...
        reward=self.get_reward(obs[0],self.action_list[-1],obs[1],obs[2],self.energy)
