...
class CarlaEnv4(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.model=torch.load("/data/carla/PythonAPI/vi_experiment_env_latency_07122023/model_train_final_local_07_11.pth")
    ...
    def action_space(self):
        ...
        return spaces.Box(low=np.array([[-1.57, 0.0]]), high=np.array([[1.57, 1.0]]),shape=(1,2), dtype=np.float64)
    ...
    def get_reward(self,collision, action,geodesic):
        ...
        if collision!=0 and self.speed_new+action[0][1]>0:
            #print("Diya")
            return -40.0
        else:
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        x_dir = math.cos(self.rotation_new+action[0][0])
        y_dir = math.sin(self.rotation_new+action[0][0])
        self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
        self.controller._control.speed = self.speed_new+action[0][1]
        self.world.player.apply_control(self.controller._control)
        obs = self._get_obs()
        if obs[1][2]>3:
            done=True
        if self.speed_new+action[0][1]>0:
