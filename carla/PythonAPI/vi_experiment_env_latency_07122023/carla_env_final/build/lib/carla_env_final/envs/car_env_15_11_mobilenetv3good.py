...
class CarlaSwitchV3goodEnv(gym.Env):
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # high_value=[1.0]*2560
        return spaces.Box(low=np.array([[-1.0]*737 for _ in range(4)]), high=np.array([[1.0]*737 for _ in range(4)]), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location, pedestrian_detected):
        ...
        return resnew[0][0], float(resnew[0][1])
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        # self.route_completion+=self.l2_distanc
        print(energy)
