...
class World(object):
    ...
    def tick(self, clock):
        ...
        if self.count % 10 == 0:
            ...
            lat = self.gnss_sensor.lat
            lon = self.gnss_sensor.lon
            self.path_points += [np.array([x, y, z, yaw, pitch, roll, lat, lon])]
        self.count += 1
    ...
    def before_destroy(self):
        # save data
        PATH = Path('/home/tzm/Projects/2_Lab/carla_910/PythonAPI/')
        name = 'path_points_'+str(time.localtime().tm_year)+str(time.localtime().tm_mon)+str(time.localtime().tm_mday) + \
            str(time.localtime().tm_hour) + \
            str(time.localtime().tm_min) + str(time.localtime().tm_sec) + '.npy'
        np.save(PATH/name, np.array(self.path_points))
        return
