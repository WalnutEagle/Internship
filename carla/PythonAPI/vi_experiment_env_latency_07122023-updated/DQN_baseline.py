...
class SaveOnEveryEpisodeCallback(BaseCallback):
    def __init__(self, log_dir: str, verbose: int = 1):
        super().__init__(verbose=verbose)
        self.n_episodes = 0
        self.log_dir = log_dir
        if os.path.isfile(self.log_dir+"monitor.csv"):
            with open(self.log_dir+"monitor.csv") as fp:
                self.n_episodes = -2
                for _ in fp:
                    self.n_episodes += 1
        self.previous_count = self.n_episodes
    
    def _on_step(self) -> bool:
        assert "dones" in self.locals, "`dones` variable is not defined, please check your code next to `callback.on_step()`"
        self.n_episodes += np.sum(self.locals["dones"]).item()
        if self.n_episodes > self.previous_count:
            self.save_path = os.path.join(self.log_dir, f"ckpts_eps{self.previous_count}.zip")
            self.model.save(self.save_path)
            self.previous_count = self.n_episodes
...
def learning_step(args):
    ...
    try:
        ...
        if resume:
            print("Hi")
            env=gym.make('carla-v0-env-router')
            env = Monitor(env, log_dir, override_existing=False)
            env.reset()
            load_path=f"{log_dir}/best_model.zip"
            #policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[256, 256]))
            model=DQN.load(load_path, env=env, device=2)
            #model.set_env(env)
            ...
            model.learn(total_timesteps=15000,callback=CallbackList([callback, SaveOnEveryEpisodeCallback(log_dir=log_dir, verbose=1)]))
            #plot_results([log_dir],15000, results_plotter.X_EPISODES, "CarlaPPO_50000_steps")
            #plt.savefig("CarlaPPO_50000_steps")
        else:
            env = gym.make('carla-v0-env-router')
            ...
            env.reset()
            # policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[256, 256]))
            model = DQN("MlpPolicy",env, verbose=1, device=2)
            # callback = CheckpointCallback(
            #     save_freq=500,
            #     save_path=log_dir,
            #     name_prefix="rl_model",
            ...
            # plot_results([log_dir],15000, results_plotter.X_EPISODES, "CarlaPPO_3000_steps")
            # plt.savefig("CarlaPPO_3000_steps")
        ##########
        # #Evaluation
        # ##########
        # vec_env=gym.make('carla-v0')
        # # # del model
        # load_path="logs/tmp/best_model.zip"
