...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            # spawn_point.location.x = 19.1
            # spawn_point.location.y = -33.7
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
...
class StateManager(object):
    ...
    def success_handler(self, timestamp):
        radius = 3 # reached goal if distance < radius
...
def spawn_cameras(world, walker, width, height, fov):
    ...
    cameras['eyelevel_rgb'] = spawn_camera(world, actor, 'rgb', width, height, fov,
...
def game_loop(args):
    ...
    try:
        ...
        cameras = {}
        cameras = spawn_cameras(world.world, world.player,
        ...
        with CarlaSyncMode(world.world, *list(cameras.values()), fps=20) as sync_mode:
            while True:
                ...
                Image.fromarray(np.uint8(eyelevel_rgb_array)).save(save_path / ('%012d_front.png' % ct))
