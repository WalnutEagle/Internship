...
def learning_step(args):
    ...
    try:
        ...
        if resume:
            print("Hi")
            env=gym.make('carla-v0-env-router-sac')
            env = Monitor(env, log_dir, override_existing=False)
            env.reset()
            load_path="logs_sac/tmp/best_model.zip"
            #policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[256, 256]))
            model=SAC.load(load_path, env=env, device=3)
            #model.set_env(env)
            ...
            env.reset()
            # policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[256, 256]))
            model = SAC("MlpPolicy",env, verbose=1, device=3)
            # callback = CheckpointCallback(
            #     save_freq=500,
            #     save_path=log_dir,
            #     name_prefix="rl_model",
