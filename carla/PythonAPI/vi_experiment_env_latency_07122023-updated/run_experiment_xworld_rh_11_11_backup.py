...
class World(object):
    ...
    def restart(self):
        ...
        # spawn_x, spawn_y, spawn_z = -31.58, 57.86, 1.5
        spawn_points = self.map.get_spawn_points()
        spawn_point = spawn_points[self.start_id]
        spawn_x, spawn_y, spawn_z = self.minimap.planner.path_pts[self.start_id][0],self.minimap.planner.path_pts[self.start_id][1],1.5
        ...
        while self.player is None:
            ...
            # [spawn_x,spawn_y]=random.choice([[-41,36],[-42,36]])
            spawn_point.location.x = spawn_x #-8
            spawn_point.location.y = spawn_y
            spawn_point.location.z = spawn_z

            # spawn_point.location.x=random.choice([self.minimap.planner.path_pts[self.start_id][0]-2,self.minimap.planner.path_pts[self.start_id][0]-1,self.minimap.planner.path_pts[self.start_id][0],self.minimap.planner.path_pts[self.start_id][0]+1,self.minimap.planner.path_pts[self.start_id][0]+2])
            # spawn_point.location.y =random.choice([self.minimap.planner.path_pts[self.start_id][1]-1,self.minimap.planner.path_pts[self.start_id][1],self.minimap.planner.path_pts[self.start_id][1]+1])
...
def game_loop(args):
    ...
    try:
        ...
        print(args.path.split('/')[-1].split('.')[-2])
        save_path = Path(args.path.split('/')[-1].split('.')[-2])
        ...
        with CarlaSyncMode(world.world, *list(cameras.values()), fps=20) as sync_mode:
            while True:
                ...
                #Image.fromarray(eyelevel_rgb_array).save(save_path / ('%012d_front.png' % ct))
