...
class World(object):
    def __init__(self, carla_world, hud, minimap, state_manager, args):
        ...
        self.spawn_points_list = [[-7.511054992675781, 59.86830520629883], [-7.735871315002441, 59.75778579711914], [-7.971434593200684, 59.697357177734375], [-8.217873573303223, 59.6616325378418], [-8.577180862426758, 59.618934631347656], [-8.810515403747559, 59.59221649169922], [-9.051197052001953, 59.56477737426758], [-9.577320098876953, 59.504817962646484], [-10.076215744018555, 59.44795608520508], [-10.639881134033203, 59.38372802734375], [-11.233997344970703, 59.31603240966797], [-11.806148529052734, 59.25083923339844], [-12.651741981506348, 59.15449905395508], [-13.23923397064209, 59.08756637573242], [-13.79289436340332, 59.02448272705078], [-14.352679252624512, 58.960693359375], [-15.218240737915039, 58.862060546875], [-15.770800590515137, 58.799110412597656], [-16.350555419921875, 58.73306655883789], [-16.92730712890625, 58.667362213134766], [-17.525726318359375, 58.599178314208984], [-18.38441276550293, 58.50133514404297], [-18.9365177154541, 58.43842697143555], [-19.48775291442871, 58.3756217956543], [-20.31932258605957, 58.2808952331543], [-20.874284744262695, 58.217674255371094], [-21.434860229492188, 58.1537971496582], [-21.98764419555664, 58.0908088684082], [-22.816242218017578, 57.996395111083984], [-23.39727210998535, 57.93019485473633], [-23.94900131225586, 57.867332458496094], [-24.498918533325195, 57.8046875], [-25.34441566467285, 57.70835876464844], [-25.922271728515625, 57.64252853393555], [-26.48884391784668, 57.57798767089844], [-27.314117431640625, 57.48397445678711], [-27.879302978515625, 57.419578552246094], [-28.43857765197754, 57.355857849121094], [-29.007835388183594, 57.290985107421875], [-29.84422492980957, 57.18991470336914], [-30.393415451049805, 57.05816650390625], [-30.915863037109375, 56.82530975341797]]
        self.restart()
    ...
    def restart(self):
        ...
        spawn_x, spawn_y = random.choice(self.spawn_points_list)
        spawn_z = 1.5
        ...
        while self.player is None:
            ...
            # [spawn_x,spawn_y]= random.choice([[-13,57],[-14,56],[-16,58.1],[-7, 59],[-20, 57],[-17,59]])
            # [spawn_x, spawn_y] = random.choice([[-7.083040237426758, 60.33004379272461], [-7.314273357391357, 60.02168655395508], [-7.511054992675781, 59.86830520629883], [-7.735871315002441, 59.75778579711914], [-7.971434593200684, 59.697357177734375], [-8.217873573303223, 59.6616325378418], [-8.577180862426758, 59.618934631347656], [-8.810515403747559, 59.59221649169922], [-9.051197052001953, 59.56477737426758], [-9.577320098876953, 59.504817962646484], [-10.076215744018555, 59.44795608520508], [-10.639881134033203, 59.38372802734375], [-11.233997344970703, 59.31603240966797], [-11.806148529052734, 59.25083923339844], [-12.651741981506348, 59.15449905395508], [-13.23923397064209, 59.08756637573242], [-13.79289436340332, 59.02448272705078], [-14.352679252624512, 58.960693359375], [-15.218240737915039, 58.862060546875], [-15.770800590515137, 58.799110412597656], [-16.350555419921875, 58.73306655883789], [-16.92730712890625, 58.667362213134766], [-17.525726318359375, 58.599178314208984], [-18.38441276550293, 58.50133514404297], [-18.9365177154541, 58.43842697143555], [-19.48775291442871, 58.3756217956543], [-20.31932258605957, 58.2808952331543]])
            [spawn_x, spawn_y] = random.choice(self.spawn_points_list)
            ...
            spawn_point.rotation.yaw = round(random.uniform(-math.pi, math.pi),1)
...
def spawn_cameras(world, walker, destination_vehicle, width, height, fov):
    ...
    # cameras['eyelevel_seg'] = spawn_camera(world, actor, 'semantic_segmentation', 1280, 720, fov,
    #                                        x_forward_eyelevel, 0.0, cam_height_eyelevel, 0.0, 0.0)
    ...
    # cameras['thirdperson_rgb'] = spawn_camera(world, actor, 'rgb', 1280, 720, fov,
    #                                        3, 0.0, 0.8, -10.0, 180.0)



    # cameras['thirdperson_side_rgb'] = spawn_camera(world, actor, 'rgb', 1280, 720, fov,
    #                                        2, -2, 0.8, -10.0, 145.0)



    # cameras['vehicle_rgb'] = spawn_camera(world, destination_vehicle, 'rgb', 1280, 720, fov,
    #                                        0.0, 1.1, 1.4, 0.0, 90.0)

    # cameras['vehicle_seg'] = spawn_camera(world, destination_vehicle, 'semantic_segmentation', 1280, 720, fov,
    #                                        0.0, 1.1, 1.4, 0.0, 90.0)


    # cameras['fix1_rgb'] = spawn_fixed_camera(world, 'rgb', 1280, 720, fov, x=-30, y=59, z=1.5, pitch=0.0, yaw=-40.0)
    # cameras['fix2_rgb'] = spawn_fixed_camera(world, 'rgb', 1280, 720, fov, x=-35, y=59, z=1.5, pitch=0.0, yaw=-80.0)
...
def get_actor_distance(actor, player_pos):
    return actor.get_location().distance(player_pos)

def get_actor_direction(actor, player):
    actor_vector = actor.get_location() - player.get_location()
    player_vector = player.get_transform().get_right_vector()
    # rotation = math.atan2(actor_pos.y-player_pos.y,actor_pos.x-player_pos.x)
    rotation = player_vector.get_vector_angle(actor_vector) - math.pi/2
...
def game_loop(args):
    ...
    try:
        ...
        print("World Loaded")
        # print('location 1: ', world.player.get_transform().location)
        ...
        # traffic_manager = util.get_traffic_manager(client, port=args.tm_port)
        ...
        vehicles_list = util.spawn_vehicles(client, world.world, CONF['num_vehicles'],  True, traffic_manager, is_move=False)

        # print("Spawning Vehicles and Pedestrians")
        # if not util.spawn_vehicles(client, world.world, CONF['num_vehicles'],  traffic_manager, vehicles_list):
        #     print("spawn_vehicles failed")
        #     return
        ...
        if not util.spawn_walkers_dense(client, world.world, dense_N, loc_center,dense_percent_disabled, CONF['percent_walking'], CONF['percent_crossing'], all_id_dict, is_walk=False):
        ...
        # print('location 2: ', world.player.get_transform().location)


        cameras = spawn_cameras(world.world, world.player, world.destination_vehicle,
                                args.width, args.height, 90)
        print("Starting Game Loop")
        state_manager.start_handler(world)

        save_path = Path("affordances_dataset")
        timestr = time.strftime("%Y%m%d-%H%M%S")
        img_save_path = save_path/(f'{timestr}/')
        npy_save_path = save_path/(f'{timestr}/')
        ...
        pedestrian_list = world.world.get_actors().filter("walker.*")

        player = world.player
        player_id = player.id
        # print("Player type:",type(player))
        # df = pd.DataFrame(columns=['distance_to_pedestrians','angle_to_pedestrians','collision_detected'])

        cur_step=0
        max_steps = 100
        images = []
        occupancy_maps = []
        with CarlaSyncMode(world.world, *list(cameras.values()), fps=20) as sync_mode:
            # while cur_step< max_steps:
            for cur_step in tqdm(range(max_steps)):
                clock.tick_busy_loop(20)
                # snapshot, eyelevel_rgb, eyelevel_ins, eyelevel_seg, eyelevel_dep, thirdperson_rgb, thirdperson_side_rgb, \
                #     vehicle_rgb, vehicle_seg, fix1_rgb, fix2_rgb = sync_mode.tick(1.0)

                # snapshot, eyelevel_rgb = sync_mode.tick(1.0)
                snapshot, eyelevel_rgb, eyelevel_ins, eyelevel_dep = sync_mode.tick(None)

                player_pos = world.player.get_transform().location
                player_pos_transform = world.player.get_transform()
                print("Starting position:",player_pos)
                # get rgb from eyelevel camera
                # eyelevel_rgb_array = util.get_image(data.pop())
                ...
                # cv2.waitKey(1)
                # thirdperson_rgb_array = util.get_image(thirdperson_rgb)
                # thirdperson_side_rgb_array = util.get_image(thirdperson_side_rgb)

                # vehicle_rgb_array = util.get_image(vehicle_rgb)
                # vehicle_seg_array = util.get_image(vehicle_seg, _type='semantic_segmentation')

                # fix1_rgb_array = util.get_image(fix1_rgb)
                # fix2_rgb_array = util.get_image(fix2_rgb)


                # get local goal
                if minimap.planner:
                    goal_x, goal_y = minimap.planner.get_next_goal(
                        pos=[player_pos.x, player_pos.y], preview_s=5)
                # if controller.parse_events(client, world, clock):
                #     return


                # get car make and model for person person view

                eyelevel_rgb_array = np.uint8(np.array(eyelevel_rgb_array))

                speed = 2.0

                forward_rotation = world.player.get_transform().rotation
                rotation_angle = math.atan(0.5)
                left_rotation = right_rotation = forward_rotation

                left_rotation.yaw = round(left_rotation.yaw+rotation_angle,1)
                right_rotation.yaw = round(right_rotation.yaw-rotation_angle,1)


                occupancy_map = np.zeros((2,2), dtype=np.float32)
                ...
                for i in range(len(rotations)):
                    ...
                    speed=1.0
                    checkpoint=0
                    occupancy_filled=False
                    while distance_covered<= math.sqrt(1.25):
                        controller._control.direction=rotations[i].get_forward_vector()
                        controller._control.speed = speed
                        world.player.apply_control(controller._control)
                        sync_mode.tick(None)
                        world.render(display)
                        pygame.display.flip()

                        distance_covered+= np.linalg.norm(np.array([world.player.get_location().x,world.player.get_location().y]) - np.array([prev_x,prev_y]))
                        # print("Here")
                        prev_x = world.player.get_location().x
                        prev_y = world.player.get_location().y
                        col_hist = world.collision_sensor.get_collision_history()
                        col_names = world.collision_sensor.get_collision_names()
                        keys_to_delete = []
                        for key,value in col_names.items():
                            if "Sidewalk" in value or "sidewalk" in value:
                        ...
                        if player.get_velocity().length() < 0.2*speed:
                            num_tries_to_move +=1
                            print("velocity_low first while loop")

                        else:
                            num_tries_to_move = 0
                        if num_tries_to_move >=15 or col_hist:
                            occupancy_map[i,checkpoint:] = 1.0
                            occupancy_filled=True
                            world.collision_sensor.history = []
                            print("Occupancy filled")
                            break

                    
                    if not occupancy_filled:
                        distance_covered=0.0
                        prev_x = player_pos.x
                        prev_y = player_pos.y
                        num_tries_to_move = 0
                        while distance_covered<=2.0:
                            controller._control.direction=forward_rotation.get_forward_vector()
                            controller._control.speed = speed
                            world.player.apply_control(controller._control)
                            sync_mode.tick(None)
                            world.render(display)
                            ...
                            if distance_covered//(checkpoint+1) == 1.0:
                                if col_hist:
                                    occupancy_map[i,checkpoint] = 1.0
                                    world.collision_sensor.history = []
                                checkpoint+=1

                            # print(player.get_velocity().length(), speed)
                            if player.get_velocity().length() < 0.2*speed:
                                num_tries_to_move +=1
                                print("velocity_low second while loop")
                            else:
                                num_tries_to_move = 0
                            if num_tries_to_move >=15 or col_hist:
                                occupancy_map[i,checkpoint:] = 1.0
                    ...
                    world.player.set_transform(player_pos_transform)
                    # batch_command = [carla.command.ApplyWalkerState(player.id, player_pos_transform, 0.0)]
                    # client.apply_batch(batch_command)
                    sync_mode.tick(None)
                    world.render(display)
                    pygame.display.flip()
                    print("velocity after reset: ",player.get_velocity().length())

                images.append(eyelevel_rgb_array)
                occupancy_maps.append(occupancy_map)

                zero_columns = np.where(np.all(occupancy_map==0.0,axis=0))[0]
                # print(zero_columns)

                # Select random direction and move 2m in that direction
                try:
                    choice = np.random.choice(zero_columns)
                except:
                    break
                distance_covered = 0
                speed = 2.0
                rotation = world.player.get_transform().rotation
                left_rotation = right_rotation = rotation
                left_rotation.yaw = round(left_rotation.yaw+math.atan(0.5/3),1)
                right_rotation.yaw = round(right_rotation.yaw-math.atan(0.5/3),1)
                angles = [left_rotation, right_rotation]
                
                prev_x = player_pos.x
                prev_y = player_pos.y
                while distance_covered <= math.sqrt(9.25):
                    controller._control.direction = angles[choice].get_forward_vector()
                    controller._control.speed=speed
                    world.player.apply_control(controller._control)
                    sync_mode.tick(None)
                ...
                # # Calculate occupancies for 4m radius in -pi/4 to pi/4 FOV
                # for i in range(len(rotation_angles)):
                #     distance_covered = 0.0
                    
                #     # Rotate player to required direction
                #     rotation = world.player.get_transform().get_forward_vector().
                #     rotation.yaw = round(rotation.yaw+rotation_angles[i],1)
                #     rotation_vector = rotation.get_forward_vector()
                    
                #     controller._control.direction = rotation_vector
                #     controller._control.speed = speed
                #     world.player.apply_control(controller._control)
                #     sync_mode.tick(None)
                #     world.render(display)
                ...
                #     prev_x = player_pos.x
                #     prev_y = player_pos.y
                #     while distance_covered <= 4.0:
                #         controller._control.direction = rotation_vector
                #         controller._control.speed = speed
                #         # print(controller._control)
                #         # control = carla.WalkerControl(rotation_vector, speed)
                #         world.player.apply_control(controller._control)
                #         sync_mode.tick(None)
                #         # world.tick(clock)
                #         world.render(display)
                ...
                #                 world.collision_sensor.history = []
                #             checkpoint+=1
                #         if player.get_velocity().length() < 0.5:
                #             num_tries_to_move +=1
                #         else:
                #             num_tries_to_move = 0
                #         if num_tries_to_move >=5:
                #             occupancy_map[i,checkpoint:] = 1.0
                #             break
                    
                #     #Return player to start location
                #     # control = carla.WalkerControl(rotation_vector,0.0)
                #     # controller._control.direction = rotation_vector
                #     # controller._control.speed = 0.0
                #     # world.player.apply_control(controller._control)

                #     # Apply control to player
                ...
                # images.append(eyelevel_rgb_array)
                # occupancy_maps[i] = occupancy_map
                
                # zero_columns = np.where(np.all(occupancy_map==0.0,axis=0))[0]
                # # print(zero_columns)

                # # Select random direction and move 4m in that direction
                # choice = np.random.choice(zero_columns)
                # distance_covered = 0
                # speed = 2.0
                # rotation = world.player.get_transform().rotation
                # rotation.yaw = round(rotation.yaw+rotation_angles[choice],1)
                # rotation_vector = rotation.get_forward_vector()
                # prev_x = player_pos.x
                # prev_y = player_pos.y
                # while distance_covered <=4.0:
                ...
                # world.player.apply_control(controller._control)
                # sync_mode.tick(None)
                # print("New player pos:",world.player.get_location())


                #annotations = []
                # annotations = get_anno(all_vehicles, cameras['eyelevel_rgb'], eyelevel_ins, eyelevel_dep, annotations, cate_id=10, fov=120)
                #annotations = get_half_anno(all_vehicles, cameras['eyelevel_rgb'], eyelevel_ins, eyelevel_dep, annotations, cate_id=10, fov=120)

                #annotations_ped = []

                # print('all_peds: ', len(all_peds))

                # annotations_ped = get_anno(all_peds, cameras['eyelevel_rgb'], eyelevel_ins, eyelevel_dep, annotations_ped, cate_id=4, fov=120)
                #annotations_ped = get_half_anno(all_peds, cameras['eyelevel_rgb'], eyelevel_ins, eyelevel_dep, annotations_ped, cate_id=4, fov=120)


                #np.save(str(save_path / ('%012d_veh.npy' % ct)), annotations)
                #np.save(str(save_path / ('%012d_ped.npy' % ct)), annotations_ped)


                # print('make and model list: ', len(annotations))

                # for anno in annotations:
                #     # brand = anno['brand'].strip('vehicle.')
                #     bbox = anno['bbox']
                ...
                #     # temp = color.split(',')
                #     # # _color = (int(temp[2]), int(temp[1]), int(temp[0]))
                #     # _color = (int(temp[0]), int(temp[1]), int(temp[2]))

                #     # eyelevel_rgb_array = cv2.rectangle(eyelevel_rgb_array, (int(bbox[0]), int(bbox[1])), (int(bbox[0]+bbox[2]), int(bbox[1]+bbox[3])), _color, 2)
                #     eyelevel_rgb_array = cv2.rectangle(eyelevel_rgb_array, (int(bbox[0]), int(bbox[1])), (int(bbox[0]+bbox[2]), int(bbox[1]+bbox[3])), (255,0,255), 2)
                #     # eyelevel_rgb_array = cv2.putText(eyelevel_rgb_array, brand, (int(bbox[0]), int(bbox[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)


                # for anno_ped in annotations_ped:
                #     bbox = anno_ped['bbox']
                #     eyelevel_rgb_array = cv2.rectangle(eyelevel_rgb_array, (int(bbox[0]), int(bbox[1])), (int(bbox[0]+bbox[2]), int(bbox[1]+bbox[3])), (0,0,255), 2)
                ...
                # eyelevel_seg_array = util.get_image(eyelevel_seg, _type='semantic_segmentation')
                # eyelevel_dep_array = util.get_image(eyelevel_dep, _type='depth')



                # # get keypoint for third person view
                # cam_world_matrix = get_matrix(cameras['thirdperson_rgb'].get_transform())
                # world_cam_matrix = np.linalg.inv(cam_world_matrix)

                # kpts = {}

                # for bone in world.player.get_bones().bone_transforms:

                #     cords = np.array([[bone.world.location.x],[bone.world.location.y],[bone.world.location.z],[1]])
                #     cam_cords = np.dot(world_cam_matrix, cords)

                #     cords_x_y_z = cam_cords[:3, :]
                #     cords_y_minus_z_x = np.concatenate([cords_x_y_z[1, :], -cords_x_y_z[2, :], cords_x_y_z[0, :]])
                #     bbox = np.transpose(np.dot(cameras['thirdperson_rgb'].calibration, cords_y_minus_z_x))
                #     camera_bbox = np.concatenate([bbox[:, 0] / bbox[:, 2], bbox[:, 1] / bbox[:, 2], bbox[:, 2]], axis=1)

                #     kpts[bone.name] = (int(camera_bbox.tolist()[0][0]),int(camera_bbox.tolist()[0][1]))


                # thirdperson_rgb_array = np.uint8(np.array(thirdperson_rgb_array))

                # for pair in pairs:
                #     a,b = pair
                #     cv2.line(thirdperson_rgb_array, kpts[a], kpts[b], (255,0,0), 2)
                
                # for _,c in kpts.items():
                #     thirdperson_rgb_array = cv2.circle(thirdperson_rgb_array, c, 1, (0,0,255), -1)


                # # get keypoint for third person side view
                # sidecam_world_matrix = get_matrix(cameras['thirdperson_side_rgb'].get_transform())
                # world_sidecam_matrix = np.linalg.inv(sidecam_world_matrix)
                ...
                #     cords = np.array([[bone.world.location.x],[bone.world.location.y],[bone.world.location.z],[1]])
                #     cam_cords = np.dot(world_sidecam_matrix, cords)

                #     cords_x_y_z = cam_cords[:3, :]
                #     cords_y_minus_z_x = np.concatenate([cords_x_y_z[1, :], -cords_x_y_z[2, :], cords_x_y_z[0, :]])
                #     bbox = np.transpose(np.dot(cameras['thirdperson_side_rgb'].calibration, cords_y_minus_z_x))
                #     camera_bbox = np.concatenate([bbox[:, 0] / bbox[:, 2], bbox[:, 1] / bbox[:, 2], bbox[:, 2]], axis=1)

                #     kpts[bone.name] = (int(camera_bbox.tolist()[0][0]),int(camera_bbox.tolist()[0][1]))


                # thirdperson_side_rgb_array = np.uint8(np.array(thirdperson_side_rgb_array))

                # for pair in pairs:
                #     a,b = pair
                #     cv2.line(thirdperson_side_rgb_array, kpts[a], kpts[b], (255,0,0), 2)
                
                # for _,c in kpts.items():
                #     thirdperson_side_rgb_array = cv2.circle(thirdperson_side_rgb_array, c, 1, (0,0,255), -1)
                ...
                # for i in pedestrian_list:
                #     actor_dist = get_actor_distance(i,player_pos) 
                #     actor_dir = get_actor_direction(i,player)
                #     if actor_dist < 6 and actor_dir !=None and (i.id != player_id):
                #         #get_actor_distance(i,player_pos) < 4 and 
                #         # print(f"Actor {i.id} distance: ",get_actor_distance(i,player_pos))
                #         # print(f"Actor {i.id} direction: ",get_actor_direction(i,player))
                #         peds_dist.append(actor_dist)
                #         peds_angle.append(actor_dir)
                #         if actor_dist < 1:
                #             collision_detected = 1.0
                
                # # print("Ped count in front:",ped_count)
                # # print(player.get_transform().rotation.yaw)
                
                # #sort peds by distance from ego walker
                # # print("Peds before sorting:",peds)
                # peds.sort(key=lambda ped: ped[0])
                # # print("Peds after sorting:",peds)

                # peds = peds[:5]

                # if len(peds) < 5:
                #     while len(peds) != 5:
                #         peds.insert(0,(0.0,0.0))
                
                
                # print(peds)

                # Save images, distance from pedestrians, collision_detected_or_not, angle_to_pedestrians
                # Image.fromarray(eyelevel_rgb_array).save(save_path / ('images/%012d_img.jpg' % ct))
                # df.loc[len(df.index)] = [peds_dist,peds_angle,collision_detected]
                

                # Image.fromarray(np.uint8(eyelevel_seg_array)).save(save_path / ('%012d_frontseg.png' % ct))
                # Image.fromarray(np.uint8(eyelevel_dep_array)).save(save_path / ('%012d_frontdep.png' % ct))


                # Image.fromarray(np.uint8(fix1_rgb_array)).save(save_path / ('%012d_fixed1.png' % ct))
                # Image.fromarray(np.uint8(fix2_rgb_array)).save(save_path / ('%012d_fixed2.png' % ct))

                
                # Image.fromarray(thirdperson_rgb_array).save(save_path / ('%012d_thirdperson.png' % ct))
                # Image.fromarray(thirdperson_side_rgb_array).save(save_path / ('%012d_thirdperson_side.png' % ct))


                # Image.fromarray(np.uint8(vehicle_rgb_array)).save(save_path / ('%012d_veh.png' % ct))
                # Image.fromarray(np.uint8(vehicle_seg_array)).save(save_path / ('%012d_vehseg.png' % ct))
                # pygame.image.save(display, str(save_path/('%012d_bev.png' % ct)))
                ct += 1
        ...
        # df.to_csv('affordances.csv',index=False)
        try:
            occupancy_maps = np.stack(occupancy_maps)
            with open(npy_save_path/'occupancies.npy', 'wb') as f:
                np.save(f,occupancy_maps)
            for i in range(len(images)):
                cv2.imwrite(str(img_save_path.joinpath("img_%012d.jpg"%i)), images[i])
        except:
            print("No data from this run: ",timestr)
