...
def learning_step(args):
    ...
    try:
        ...
        if resume:
            ...
            model.learn(total_timesteps=15000,callback=CallbackList([callback,WandbCallback(gradient_save_freq=1500)]))
            ...
            env.reset()
            policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[32, 32]))
            model = PPO("MlpPolicy",env, policy_kwargs=policy_kwargs, verbose=1,gamma=0.5, device=1)
            callback = SaveOnBestTrainingRewardCallback(check_freq=1500, log_dir=log_dir)
        ...
        # # # del model
        # load_path="logs_11_18_14092023/tmp/best_model.zip"
        ...
        try:
            with open('task_metrics_residual_policy.pkl','wb') as f:
                pickle.dump(vec_env.task_metrics, f)
            vec_env.close()
        except:
            env.close()
        pygame.quit()
