...
def spawn_cameras(world, walker, destination_vehicle, width, height, fov):
    ...
    actor = walker
    # eye-level camera
    x_forward_eyelevel = 0.3  # 0.4
    cam_height_eyelevel = 0.8  # 0.5
    cameras['eyelevel_rgb'] = spawn_camera(world, actor, 'rgb', 1280, 720, fov,
                                           x_forward_eyelevel, 0.0, cam_height_eyelevel, 0.0, 0.0)

    cameras['eyelevel_ins'] = spawn_camera(world, actor, 'instance_segmentation', 1280, 720, fov,
                                           x_forward_eyelevel, 0.0, cam_height_eyelevel, 0.0, 0.0)

    cameras['eyelevel_seg'] = spawn_camera(world, actor, 'semantic_segmentation', 1280, 720, fov,
                                           x_forward_eyelevel, 0.0, cam_height_eyelevel, 0.0, 0.0)

    cameras['eyelevel_dep'] = spawn_camera(world, actor, 'depth', 1280, 720, fov,
                                           x_forward_eyelevel, 0.0, cam_height_eyelevel, 0.0, 0.0)
...
from utils_carmake import get_anno
    ...
    try:
        ...
        with CarlaSyncMode(world.world, *list(cameras.values()), fps=20) as sync_mode:
            while True:
                ...
                annotations = []
                annotations = get_anno(all_vehicles, cameras['eyelevel_rgb'], eyelevel_ins, eyelevel_dep, annotations, cate_id=10, fov=90)
