...
def learning_step(args):
    ...
    try:
        ...
        vec_env=gym.make('carla-v0-eval-mulreward')
        # vec_env.reset()
        for i in range(0,360,3):
            load_path=os.path.join(args.model_path, f"tmp/ckpts_eps{i}.zip")
            model=PPO.load(load_path, env=vec_env)
            # model = PPO("MlpPolicy", vec_env, policy_kwargs=policy_kwargs, verbose=1, device=1)
