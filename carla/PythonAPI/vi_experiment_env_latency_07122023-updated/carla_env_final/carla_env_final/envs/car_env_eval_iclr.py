...
sys.path.append('/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/carla_env_final/carla_env_final/envs/')
...
class ICLREvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0, 0,0] for _ in range(4)])
        self.sampled_latency=0
        self.q=0
        self.ct=0
        self.prev_angle=0.0
        self.global_model = torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/iclr_baseline/train_final_cloud1_07_11.pth").module
        self.global_model = self.global_model.to(torch.device('cuda'))
        self.global_model.eval()
        self.router = torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/iclr_baseline/routing_model_epoch200_lr0.01_thres1.0.pth").module
        self.router = self.router.to(torch.device('cuda'))
        self.router.eval()
        self.model=torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/iclr_baseline/model_train_final_local_07_11.pth").module
        ...
        self.result = cv2.VideoWriter(f"new_experiments/iclr_baseline/rollout_ablation_iclr_baseline_high_path_21_new_new.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter(f"new_experiments/iclr_baseline/minimaps/minimap_iclr_baseline_high_path_21_new_new.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location):
        ...
        res,ft=self.model(img_tensor.to(torch.device('cuda')), location_tensor.to(torch.device('cuda')))
        # print(res.get_device())
        res_tens = res
        # print(res[0][0],res[0][1], type(res[0][0]),type(res[0][1]))
        # res=res.cpu().detach().numpy()
        self.action_list = np.vstack((self.action_list, np.array([res[0][0].cpu().detach().numpy(),res[0][1].cpu().detach().numpy(),0.0])))
        energy=0.15
        return self.action_list[-4:],energy, res_tens, ft
        # return self.action_list[-4:]
    ...
    def expert_policy(self,pedestrian_detected,image,location):
        ...
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
        res,ft=self.global_model(img_tensor.to(torch.device('cuda')), location_tensor.to(torch.device('cuda')))
        # print(res[0],res[1])
        # res=res.cpu().detach().numpy()
        if pedestrian_detected==0:
            self.speed_new = res[0][1].cpu().detach().numpy()
            self.rotation_new=res[0][0].cpu().detach().numpy()
        else:
            self.speed_new = 0
            self.rotation_new=0
    ...
    def pre_act(self):
        ...
        a,energy,res_tens,ft=self.local_policy(self.eyelevel_rgb_array,waypt)
        switch = self.router(res_tens, ft)
        # print("Router output:",switch)
        switch = int(switch > 0.7)
        
        return a,pedestrian_detected,energy,waypt, switch
    ...
    def _get_obs(self):
        ...
        cv2.imshow("walker camera View Selective Query", self.eyelevel_rgb_array)
        cv2.waitKey(1)
        #self.sync_mode.tick(None)
        # self.result.write(self.eyelevel_rgb_array)
    ...
    def get_reward(self,collision, action,geodesic,dir_r, energy):
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        #obs = self._get_obs()
        start = time.time_ns()
        a, pedestrian_detected,energy,waypt, switch=self.pre_act()
        ...
        # # print(np.sort(col_count))
        # pedestrian_detected = 1.0 if self.ped_count > 1 else 0.0
        # pathxclose,pathyclose=self.minimap.planner.get_next_goal(np.array([self.world.player.get_location().x,self.world.player.get_location().y]),preview_s=5)
        ...
        if switch==0:
            ...
            x_dir = math.cos(latent_dir)
            y_dir = math.sin(latent_dir)
            self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
            ...
            print("Inside Cloud")
        # self.local_policy(self.eyelevel_rgb_array,pathxclose,pathyclose,pedestrian_detected)
        # if action==0:
        #     #angle_degrees * (math.pi / 180)
        ...
        #     print("Inside Local")
        # elif action==1:
        #     # action_expert,Flop=self.expert_policy(pedestrian_detected)
        #     # an=action_expert[0]
        #     self.expert_policy(pedestrian_detected)
        #     latency=random.uniform(0,0.2)
        ...
        reward=self.get_reward(obs[0],self.action_list[-1],obs[1],obs[2],energy)
        self.total_latency += latency
        self.energy_consumption += energy

        # Add legends to video
        # mode = "Selective Query"
        mode_loc = (5, 15)
        infraction_loc = (5,40)
        energy_loc = (5,65)
        latency_loc = (5, 90)
        ...
        # overlay = cv2.rectangle(overlay, top_left, bottom_right, (97, 100, 100), -1)  # A filled rectangle
        ...
        # # Add Latency
        # updated_img = cv2.putText(updated_img, f'FPS: ', latency_loc, font,
        #                             fontScale, color, thickness, cv2.LINE_AA)
        ...
        for index in range(self.current_wp_index, min(self.current_wp_index+5,len(self.waypts))):
    ...
    #print(p)
        ...
        #print(p)
            ...
            print("Task Metrics:")
            print("Collision Count:",self.infraction_ped_collision)
            ...
            self.task_metrics.append([self.destination_success,(self.percentage_route_completion*(0.5**self.infraction_ped_collision)*(1-(self.energy_consumption-(0.15*(self.local_step_counter+self.global_step_counter)))/(1.5*(self.local_step_counter+self.global_step_counter)))), self.infraction_ped_collision,self.energy_consumption,self.total_latency,self.percentage_route_completion, self.route_deviation, self.route_length*self.percentage_route_completion/100000.0, episode_time, self.local_step_counter, self.global_step_counter])
            # self.episode_start_time = time.time()
            #print("Energy",)
    ...
    def close(self):
        ...
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Ecological Navigation Score", "Infraction Count (Pedestrian Collisions)", "Energy Consumption (J)", "Total Latency (s)","Route Completion (%)", "Avg. Route Deviation (m)", "Distance covered (km)", "Time taken (s)", "Number of steps taken by local model", "Number of steps taken by global model"])
        df.to_csv(f'new_experiments/iclr_baseline/high_setup/task_metrics_ablation_iclr_baseline_high_path_21_new_new.csv',index=False)
