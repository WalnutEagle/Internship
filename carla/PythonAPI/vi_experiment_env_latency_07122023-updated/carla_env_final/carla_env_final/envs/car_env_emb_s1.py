...
class CarlaEvalEnvrouters1(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0, 0,0]])
        ...
        self.model=torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_reg_s1_worst_worst.pth").to(torch.device('cuda'))
        self.model.eval()
        ...
        self.result = cv2.VideoWriter("new_experiments/logs_4_3_wo_history/high_setup/rollout_ablation_logs_4_3_wo_history.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("new_experiments/logs_4_3_wo_history/high_setup/minimaps/minimap_logs_4_3_wo_history.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location,pedestrian_detected):
        ...
        res=self.model(img_tensor.to(torch.device('cuda:0')), location_tensor.to(torch.device('cuda:0')))
        emb=self.model(img_tensor.to(torch.device('cuda:0')), location_tensor.to(torch.device('cuda:0')),True).squeeze(0)
    ...
    def pre_act(self):
        ...
        a,emb,energy=self.local_policy(self.eyelevel_rgb_array,waypt,pedestrian_detected)
        
        return a,emb,pedestrian_detected,energy,waypt
    def _get_obs(self):
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        #obs = self._get_obs()
        start = time.time_ns()
        a, emb,pedestrian_detected,energy,waypt=self.pre_act()
    ...
    #print(p)
        ...
        return np.concatenate([np.array(emb), self.action_list[-1]], axis=0).reshape(1,51), reward, done, truncated,self.info
    ...
    def close(self):
        ...
        df.to_csv('new_experiments/logs_4_3_wo_history/high_setup/task_metrics_ablation_logs_4_3_wo_history_high.csv',index=False)
