...
sys.path.append('/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/spinn/')
...
class SPINNEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.model = torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/spinn/spinn_RegNet_EE4_epochs200.pth").module
        ...
        self.result = cv2.VideoWriter(f"rebuttal_experiments/spinn_baseline/rollout_ablation_spinn_dense_path_32_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter(f"rebuttal_experiments/spinn_baseline/minimaps/minimap_spinn_dense_path_32_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location):
        img_tensor = self.normalize((self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560])).unsqueeze(0).to(torch.device('cuda')) # self.transform2
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0).to(torch.device('cuda'))
        res, flag=self.model.infer(img_tensor, location_tensor, 0.5, env="mobile")
        # res=res.cpu().detach().numpy()
        energy_multiplier = 0
        if not isinstance(flag, list):
            if flag == 1:
                energy_multiplier = 0.005
            elif flag == 2:
                energy_multiplier = 0.08
            elif flag == 3:
                energy_multiplier = 0.84
            else:
                energy_multiplier = 1.0
        energy=0.15*energy_multiplier
        return res, energy, flag
    # def expert_policy(self,pedestrian_detected):
    #     if pedestrian_detected==0:
    #         # start_time = time.time()
    #         self.speed_new = 1.5
    #         self.rotation_new=self._run_global_policy()
    #         # time_taken = time.time()-start_time
    #         # local_energy_consumption = 10*time_taken
    #         # self.local_energy_consumption+= local_energy_consumption
    #     # elif self.last_detected==1:
    ...
    #     self.expert_policy_list = np.vstack((self.expert_policy_list, np.array([self.rotation_new, self.speed_new])))        
    #     return self.expert_policy_list[-15:-5]

    def expert_policy(self, pedestrian_detected, x, location,flag):
        # print(x.shape)
        # x = x.unsqueeze(0).to(torch.device('cuda'))
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0).to(torch.device('cuda'))
        # print("flag[0][0]: ",flag[0])
        res,flag1 = self.model.infer(x,location_tensor, 0.5,env="cloud",actions=flag[0],confidenceScores=flag[1])
        # print(res)
        # res.cpu().detach().numpy()
        if pedestrian_detected==0:
            self.speed_new = res[0][0]
            self.rotation_new=res[0][1]
        else:
            self.speed_new = 0
            self.rotation_new=0
    ...
    def pre_act(self):
        ...
        a,energy,flag=self.local_policy(self.eyelevel_rgb_array,waypt)
        return a,pedestrian_detected,energy,waypt,flag
    def _get_obs(self):
        ...
        cv2.imshow("walker camera View SPINN", self.eyelevel_rgb_array)
        cv2.waitKey(1)
        #self.sync_mode.tick(None)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        start = time.time_ns()
        flag = None
        a, pedestrian_detected,energy,waypt,flag=self.pre_act()
        ...
        if isinstance(flag, list):
            ...
            #     print("List empty")
            # print(flag[0],flag[1])
            model_action, e = self.expert_policy(pedestrian_detected, a,waypt,flag)
            mode = "Cloud"
            inference_latency += (time.time_ns() - start)/(10**9)

        # x_dir=math.cos(model_action[0])
        # y_dir=math.sin(model_action[0])
        # self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
        # self.controller._control.speed=model_action[1]
    ...
    def close(self):
        ...
        df.to_csv(f'rebuttal_experiments/spinn_baseline/task_metrics_ablation_spinn_dense_path_32_2.csv',index=False)
