...
class CarlaSwitchEnv(gym.Env):

    metadata = {'render_modes': ['human']}
    def __init__(self, args,vehicles_list,all_id_dict):
        #self.server=setup1()
        self.client, self.world, self.server,self.minimap, self.state_manager, self.traffic_manager,self.controller,self.display, self.sparse_id_dict, self.dense_id_dict, self.vehicles_list,self.s=setup()        #print(self.s)
        self.total_reward = 0
        self.l2_distance=0
        ...
        self.model=torch.load("/data2/kathakoli/carla/PythonAPI/vi_experiment_env_latency_07122023/model_train_final_local_07_11.pth")
    ...
    def local_policy(self, image, pathclosex, pathclosey,pedestrian_detected):
        ...
        angle,speed=self.model(img_tensor, location_tensor)
        self.action_list = np.vstack((self.action_list, np.array([angle,speed,0.0])))
        self.energy=0.15
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        if action==0:
            ...
            #     self.action_list = np.vstack((self.action_list, np.array([self.action_list[-1][0],self.action_list[-1][1],0.0])))
            ...
            # latent_dir=self.action_list[-1][0]
            # latent_speed=self.action_list[-1][1]
