...
class CarlaEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.expert_policy_list=np.array([[0, 0] for _ in range(15)])
        self.last_action=[0,0]
        # self.expert_policy_list=np.array([[0, 0,0] for _ in range(4)])
        ...
        self.result = cv2.VideoWriter("rollout_ablation_mulped_04_10.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # return spaces.Box(low=np.array([[-3.14, 0.0,0.0] for _ in range(4)]), high=np.array([[3.14, 2.0,10.0] for _ in range(4)]),shape=(4,3), dtype=np.float64)
        #0.0,, np.inf 
    ...
    #     # assuming action is normalized to [-1, 1]
    #     # speed should be normalized between 0 and 1
    #     if collision!=0 and self.world.player.get_velocity().length()>1e-5:
    #          return -20.0
    #     else:
    #         extreme_action0_rw = 1.0 if abs(action[0][0]/3.14) < 0.97 else 0.0
    #         extreme_action1_rw = 1.0 if abs(float(action[0][1])/2) < 0.97 else 0.0
    #         extreme_actions_reward = (extreme_action0_rw*extreme_action1_rw)**.5
    #         #print(geodesic)
    #         geodesic_rw = 1.0 - math.tanh(geodesic)
    #         #print(geodesic_rw)
    #         #speed=(action[0][1])/2x
    #         speed=self.world.player.get_velocity().length()
    #         #print(geodesic_rw)
    #         #return geodesic_rw
    #         #print((geodesic_rw*speed*extreme_actions_reward)**(1.0/3.0))
    #         return (geodesic_rw*speed*extreme_actions_reward)**(1.0/3.0)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        x_dir = math.cos(self.last_action[0])
        y_dir = math.sin(self.last_action[0])
        # x_dir = math.cos(action[0][0])
        # y_dir = math.sin(action[0][0])
        self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
        self.controller._control.speed = self.last_action[1]
        # self.controller._control.speed = float(action[0][1])
        ...
        reward=self.get_reward(obs[1][1],self.last_action,obs[1][2])
        self.prev_x=self.minimap.player_pos.x
        self.prev_y=self.minimap.player_pos.y
        # if self.world.player.get_velocity().length()>1e-5:
        ...
        if done or truncated:
            ...
            self.task_metrics.append([self.destination_success,self.infraction_ped_collision,self.local_energy_consumption,self.distance_covered, self.percentage_route_completion, self.route_deviation])
            #print("Energy",)
    ...
    def close(self):
        ...
        self.result.release()
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Local Power Consumption (J)", "Distance travelled (m)","Route Completion (%)", "Avg. Route Deviation (%)"])
        df.to_csv('task_metrics_ablation_mulped_04_10.csv',index=False)
