...
from Model_cust import EdgeModel
...
# from local_model_resnet18 import ClassificationNetwork
...
class DeepCODEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.global_model = torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth").to(torch.device('cuda'))
        self.global_model.eval()
        # self.model = CustomResNet18()
        self.model = torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/CVPR_customLinear_21_11_data_08_11_epoch80_lr1emin4_nopretrain_normal.pth")
        self.model = self.model.to(torch.device('cuda'))
        self.model.eval()

        ckpt = torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_best.ckpt")
        self.deepcod = EdgeModel()
        self.deepcod.load_state_dict(ckpt['model_state_dict'])
        self.deepcod.eval()
        # self.split = 5
        ...
        self.result = cv2.VideoWriter("new_experiments/deepcod_baseline/high_setup/rollout_ablation_deepcod_baseline.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("new_experiments/deepcod_baseline/high_setup/minimaps/minimap_deepcod_baseline.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        # return spaces.Box(low=np.array([[-3.14, 0.0] for _ in range(10)]), high=np.array([[3.14, 2.0] for _ in range(10)]),shape=(10,2), dtype=np.float64)
        return spaces.Box(low=np.array([-3.14, 0.0,0.0]), high=np.array([3.14, 2.0,1.0]), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def _get_obs(self):
        ...
        cv2.imshow("walker camera View DQN Baseline", self.eyelevel_rgb_array)
        cv2.waitKey(1)
        #self.sync_mode.tick(None)
    ...
    def reset(self,seed=None, options=None):
        ...
        return self.action_list[-1],self.info
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        #     print("Inside Local")
        # elif action==1:
        mode = "Deep Compressive Offloading + Cloud (Latent)"
        transmit_latency = 0.0
        tensor_transform = transforms.ToTensor()
        resized_image = tensor_transform(self.eyelevel_rgb_array[:,80:560,:])  # Cropping, adjust as needed
        # print(resized_image.shape)
        normalized_image = normalize(resized_image.float()/ 255.0, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normalization
        resized_normalized_image = resize(normalized_image, size=[224, 224])
        resized_normalized_image = resized_normalized_image.unsqueeze(0)
        start = time.time_ns()
        img_encoded = self.deepcod.encoder(resized_normalized_image)
        deepcod_latency = (time.time_ns() - start)/(10**9)
        start = time.time_ns()
        img_decoded = self.deepcod.decoder(img_encoded)
        deepcod_latency += (time.time_ns() - start)/(10**9)
        start = time.time_ns()
        action_expert,e=self.expert_policy(pedestrian_detected,img_decoded,waypt)
        inference_latency = (time.time_ns() - start)/(10**9)
        #an=action_expert[0]
        ...
        energy=e
        latency += deepcod_latency+inference_latency
    ...
    #         truncated = True
        ...
        return self.action_list[-1], reward, done, truncated,self.info
    ...
    def close(self):
        ...
        df.to_csv('new_experiments/deepcod_baseline/high_setup/task_metrics_ablation_deepcod_baseline.csv',index=False)
