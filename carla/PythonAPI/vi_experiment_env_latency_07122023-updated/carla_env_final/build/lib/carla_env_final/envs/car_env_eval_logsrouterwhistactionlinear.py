...
class CarlaEvalHistLinearEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.ct=0
        self.prev_angle=0.0
        self.global_model = torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth").module.to(torch.device('cuda'))
        ...
        self.model=torch.load("/data2/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_train_final_local_07_11.pth").to(torch.device('cuda'))
        self.model.eval()
        ...
        self.result = cv2.VideoWriter("new_experiments/logsrouterwhistactionlinear/high_setup/rollout_ablation_logsrouterwhistactionlinear.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("new_experiments/logsrouterwhistactionlinear/high_setup/minimaps/minimap_logsrouterwhistactionlinear_high.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([[-3.14, 0.0,0.0]for _ in range(4)]), high=np.array([[3.14, 2.0,1.0]for _ in range(4)]), dtype=np.float64)
        #0.0,, np.inf 
    ...
    def local_policy(self, image, location,pedestrian_detected):
        ...
        img_tensor = self.normalize(self.transform2(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560])).unsqueeze(0)
    ...
    def expert_policy(self,pedestrian_detected,image,location):
        ...
        # else:
        # img_tensor = self.normalize(self.transform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))[:,:,80:560]).unsqueeze(0)
        # location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0)
    ...
    def get_reward(self,collision, action,geodesic,dir_r, energy):
        ...
        return (geodesic_rw+speed+energy_dis)/3.0
    ...
    def close(self):
        ...
        df.to_csv('new_experiments/logsrouterwhistactionlinear/high_setup/task_metrics_ablation_logsrouterwhistactionlinear.csv',index=False)
