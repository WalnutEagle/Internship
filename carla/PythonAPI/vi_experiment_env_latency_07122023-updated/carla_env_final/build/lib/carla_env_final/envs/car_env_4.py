...
class CarlaEnv(gym.Env):
    ...
    def observation_space(self, *args, **kwargs):
        ...
        distance_covered=spaces.Box(low=0.0,high=np.inf, dtype=np.float32)
        ...
        ##pedestrian/not, collision/not, distance covered
    ...
    def _get_obs(self):
        ...
        pedestrian_detected = 1 if self._get_num_people(self.eyelevel_rgb_array,self.model_yolo) > 0 else 0
        collision_detected=len(self.world.collision_sensor.get_collision_history())
        distance_covered=self.time_covered*self.speed
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        reward = 0
        #info = dict()
        l2_distance = np.linalg.norm(np.array([self.minimap.player_pos.x,self.minimap.player_pos.y]) - np.array([self.prev_x,self.prev_y]))
        reward+=100*(l2_distance)
        self.route_completion+=l2_distance
        if obs[1]!=0:
        ...
        if done or truncated:
            ...
            self.task_metrics.append([self.count_epi,len(self.world.collision_sensor.get_collision_history()),self.route_completion,self.energy])
            #print("Energy",)
    ...
    def _run_global_policy(self):
        ...
        rotation = math.atan2(ego_world_pos[0][1]-local_goal[0][1], ego_world_pos[0][0]-local_goal[0][0])
