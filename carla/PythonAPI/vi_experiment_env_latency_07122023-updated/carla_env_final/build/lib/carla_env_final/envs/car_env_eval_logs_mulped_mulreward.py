...
class CarlaEvalMulRewardEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.task_metrics=[]
        self.count_epi=0
        self.action_list=np.array([[0, 0,0] for _ in range(5)])
        # self.all_policies=
        self.q=0
        self.ct=0
        self.prev_angle=0.0
        ...
        #self.all_policies=np.array([[0, 0,0,0,np.inf] for _ in range(4)])
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=np.array([[-3.14, 0.0,0.0] for _ in range(5)]), high=np.array([[3.14, 2.0,1.0] for _ in range(5)]),shape=(5,3), dtype=np.float64)
        #0.0,, np.inf 
    ...
    #     return seed
    def local_policy(self, image, pathclosex, pathclosey,pedestrian_detected):
    ...
    def expert_policy(self,pedestrian_detected):
        ...
        # self.all_policies = np.vstack((self.all_policies, np.array([self.rotation_new, self.speed_new,1.0,self.sampled_latency,self.sampled_latency])))
        # for i in range(len(self.all_policies)):
        #     # print(self.all_policies[i],"Hello")
        #     # print(type(self.all_policies[i]))
        #     if self.all_policies[i][4]==0:
        #         self.action_list = np.vstack((self.action_list,self.all_policies[i][:4]))
        #         # self.expert_policy_list.append(self.all_policies[i][:3])
        #         self.all_policies[i][4]=np.inf
        #     else:
        #         self.all_policies[i][4]-=1
        ...
        #self.action_list = np.vstack((self.action_list, np.array([self.rotation_new,self.speed_new,1.0])))
        # self.action_list[-1][3]=int(np.random.normal(5, 2.5, 1))
        #np.array([self.rotation_new, self.speed_new,1.0])     
        #return self.expert_policy_list[-4:]
        #flop=3.6
        #return self.action_list[-1], flop
    # def pre_act(self):
    ...
    #     pathxclose,pathyclose=self.minimap.planner.get_next_goal(np.array([self.world.player.get_location().x,self.world.player.get_location().y]))
    #     a=self.local_policy(self.eyelevel_rgb_array,pathxclose,pathyclose,pedestrian_detected)
        
    #     return a,pedestrian_detected
    def _get_obs(self):
    ...
    def reset(self,seed=None, options=None):
        ...
        return self.action_list[-5:],self.info
    ...
    def get_reward(self,collision, action,geodesic,dir_r, energy):
        ...
        if collision!=0 and action[1]>1e-5:
            ...
            print("Geodesic",geodesic_rw)
                #print(geodesic)
                #speed=(action[0][1])/2x
            power_dis=(1.0-energy/4.25)#Flop/Max Flop
            speed=self.world.player.get_velocity().length()/2
            #Latency_final=(1.0-latency/10)
            # print("Speed",speed)
            #return geodesic_rw
        return ((geodesic_rw*speed*extreme_actions_reward*power_dis)**(1.0/4.0))
        #return (geodesic_rw*speed)**(1.0/3.0)
        #return geodesic_rw
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        #action=obs[0][-1][:2]
        #obs = self._get_obs()
        #a, pedestrian_detected=self.pre_act()
        self.ped_count=0
        player_pos = self.world.player.get_location()
        ped_distances = []
        for i in self.pedestrian_list:
        ...
        #print(action)
        # if self.sampled_latency>0:
        #     self.action_list= np.vstack((self.action_list, np.array([self.action_list[-1][0],self.action_list[-1][1],1.0])))
        ...
        #     print("Inside Cloud")
        #     Flop=3.6/self.val
        #     self.sampled_latency-=1
        # elif self.sampled_latency==0:
        ...
        #     self.local_policy(self.eyelevel_rgb_array,pathxclose,pathyclose,pedestrian_detected)
        #     self.sampled_latency=int(np.random.normal(5, 2.5, 1))
        #     self.val=self.sampled_latency
        #     self.action_list[-1][0]=self.action_list[-2][0]
        #     self.action_list[-1][1]=self.action_list[-2][1]
        ...
        if action==0:
            ...
            self.action_list[-1][1]=0.0
            self.action_list[-1][2]=1.0
            #print("Player Position before",self.minimap.player_pos)
            for i in range(3):
                #print("Player Positions",self.world.player.get_location())
                x_dir = math.cos(self.action_list[-1][0])
                y_dir = math.sin(self.action_list[-1][0])
                self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
                self.controller._control.speed = self.action_list[-1][1]
                self.world.player.apply_control(self.controller._control)
                self.sync_mode.tick(None)
                #print(self.action_list[-1][0],self.action_list[-1][1])
                self.action_list = np.vstack((self.action_list, np.array([0.0,0.0,1.0])))
            #print("Player Position after",self.world.player.get_location())
            #self.world.tick(self.clock)
            # self.sync_mode.tick(None)
            #self.render()
            start = time.time_ns()
    ...
    #print(p)
        ...
        return self.action_list[-5:], reward, done, truncated,self.info
    ...
    def close(self):
        # print("Inside close")
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Navigation Score", "Infraction Count (Pedestrian Collisions)", "Energy Consumption (J)", "Total Latency (s)","Route Completion (%)", "Avg. Route Deviation (m)", "Distance covered (km)", "Time taken (s)", "Number of steps taken by local model", "Number of steps taken by global model"])
        df = df.groupby(np.arange(len(df))//3).mean()
        df.to_csv('parkour_plots/logs_mulped_mulreward/task_metrics_mulped_mulreward.csv',index=False)
        self.client.apply_batch([carla.command.DestroyActor(x)
