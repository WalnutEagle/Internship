...
class NeurosurgeonEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.model = torch.load("/data/sandesh/carla/PythonAPI/vi_experiment_env_latency_07122023/model_10_11_RegNet.pth")
        ...
        self.result = cv2.VideoWriter(f"new_experiments/neurosurgeon_baseline/rollout_ablation_neurosurgeon_split{self.split}_dense_path_32_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter(f"new_experiments/neurosurgeon_baseline/minimaps/minimap_neurosurgeon_split{self.split}_dense_path_32_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location):
        ...
        res=self.model(img_tensor, location_tensor, self.split, "mobile")
    ...
    def expert_policy(self, pedestrian_detected, x, location):
        # print(x.shape)
        # x = x.unsqueeze(0).to(torch.device('cuda'))
        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0).to(torch.device('cuda'))
        res = self.model(x,location_tensor,self.split, "cloud")
    ...
    def _get_obs(self):
        ...
        cv2.imshow("walker camera View Neurosurgeon", self.eyelevel_rgb_array)
        cv2.waitKey(1)
        #self.sync_mode.tick(None)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        start = time.time_ns()
        model_action, e = self.expert_policy(pedestrian_detected, a,waypt)
        inference_latency += (time.time_ns() - start)/(10**9)
    ...
    def close(self):
        ...
        df.to_csv(f'new_experiments/neurosurgeon_baseline/task_metrics_ablation_neurosurgeon_split{self.split}_dense_path_32_2.csv',index=False)
