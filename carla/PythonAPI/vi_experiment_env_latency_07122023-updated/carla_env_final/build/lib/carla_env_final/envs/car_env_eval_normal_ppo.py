...
class CarlaEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.result = cv2.VideoWriter("rollout_ablation_normal_ppo.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def _get_obs(self):
        ...
        distance_covered=np.linalg.norm(np.array([pathxclose,pathyclose])-np.array([self.world.player.get_location().x,self.world.player.get_location().y]))
        # path_next_x,path_next_y=self.minimap.planner.get_next_goal(pos=[pathxclose,pathyclose],preview_s=2)
        # #print(path_next_x,path_next_y)
    ...
    def reset(self,seed=None, options=None):
        ...
        self.local_energy_consumption = 0.0
        self.cloud_policy_time = 0.0
    ...
    def get_reward(self,collision, action,geodesic,dir_r, Flop):
        ...
        if collision!=0 and action[1]>1e-5:
            ...
            #print(geodesic_rw)
            #speed=(action[0][1])/2x
            power_dis=(1.0-Flop/19.3)#Flop/Max Flop
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        reward=self.get_reward(obs[0],self.action_list[-1],obs[1],obs[2],Flop)
        ...
        #print(p)
            ...
            self.task_metrics.append([self.destination_success,self.infraction_ped_collision,self.num_flops,self.total_latency,self.percentage_route_completion, self.route_deviation, episode_time])
            # self.episode_start_time = time.time()
            #print("Energy",)
    ...
    def close(self):
        ...
        self.result.release()
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Number of FLOPS (GFLOPS)", "Total Latency (s)","Route Completion (%)", "Avg. Route Deviation (m)", "Time taken (s)"])
        df.to_csv('task_metrics_ablation_normal_ppo.csv',index=False)
