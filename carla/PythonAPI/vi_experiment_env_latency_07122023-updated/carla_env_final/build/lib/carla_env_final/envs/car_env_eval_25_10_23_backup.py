...
class CarlaEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        self.result = cv2.VideoWriter("rollout_ablation_mulped_latency.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    #     extreme_action1_rw = 1.0 if abs(float(action[1])/2) < 0.97 else 0.0
    #     extreme_actions_reward = (extreme_action0_rw*extreme_action1_rw)**.5
    #     #print(geodesic)
    #     geodesic_rw = 1.0 - math.tanh(geodesic)
    #     #print(geodesic_rw)
    ...
    def close(self):
        ...
        self.result.release()
        df = pd.DataFrame(self.task_metrics, columns=["Success Rate", "Infraction Count (Pedestrian Collisions)", "Local Power Consumption (J)", "Distance travelled (m)","Route Completion (%)", "Avg. Route Deviation (m)"])
        df.to_csv('task_metrics_ablation_mulped_latency.csv',index=False)
