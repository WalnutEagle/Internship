...
class ImitationEvalEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        ...
        # self.model = CustomResNet18() local_model_10_11_mobilenet
        ...
        self.result = cv2.VideoWriter("new_experiments/local/crowd_setup/rollout_ablation_local_model_10_11_mobilenet_crowdSetup_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
        self.minimap_video = cv2.VideoWriter("new_experiments/local/crowd_setup/minimaps/minimap_local_model_10_11_mobilenet_crowdSetup_2.avi",cv2.VideoWriter_fourcc(*'MJPG'),20,(640,360))
    ...
    def local_policy(self, image, location):
        ...
        res=self.model(img_tensor, location_tensor)
    ...
    def _get_obs(self):
        ...
        cv2.imshow("walker camera View imitation", self.eyelevel_rgb_array)
        cv2.waitKey(1)
        #self.sync_mode.tick(None)
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        # model_action, flop = self.expert_policy(a, location_tensor, pedestrian_detected)
        # model_action = a[-1][0]
        # print(model_action,a[-1][1])
        # print("model action: ",a)
        x_dir=math.cos(a[-1][0])
        y_dir=math.sin(a[-1][0])
        self.controller._control.direction=carla.Vector3D(x_dir,y_dir,0.0)
        self.controller._control.speed=float(a[-1][1])
    ...
    def close(self):
        ...
        df.to_csv('new_experiments/local/crowd_setup/task_metrics_ablation_local_model_10_11_mobilenet_crowdSetup_2.csv',index=False)
