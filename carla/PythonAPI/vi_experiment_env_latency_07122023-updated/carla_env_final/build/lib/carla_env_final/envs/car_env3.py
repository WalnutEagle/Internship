...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, args,vehicles_list,all_id_dict):
        self.client, self.world, self.server, self.minimap, self.state_manager, self.traffic_manager,self.controller,self.display=setup()
        self.collision_hist = []
        #self.actor_list = []
        self.frame_step = 0
        #self.out_of_loop = 0
        ...
        # print("Loading World")
        # self.world = World(self.client.get_world(), self.hud, self.minimap, self.state_manager, args)

        # print("World Loaded")
        # # print('location 1: ', world.player.get_transform().location)
        # if args.weather in util.WEATHER:
        #     self.weather = util.WEATHER[args.weather]
        # else:
        #     self.weather = util.WEATHER['ClearNoon']
        # self.world.world.set_weather(self.weather)
        ...
        # self.settings = self.world.world.get_settings()
        ...
        # self.world.world.apply_settings(self.settings)
    ...
    def observation_space(self, *args, **kwargs):
        ...
        return spaces.Box(low=0.0, high=255.0, shape=(self.im_height, self.im_width, 3), dtype=np.uint8)
    ...
    def reset(self,seed=None, options=None):
        ...
        #self.front_image_Queues=[]
        #self.actor_list.append(self.world.player)
        if(self.count>1):
        ...
        #self.world.world.on_tick(self.front_image_Queue.put)
        #self.cameras['eyelevel_rgb'].listen(self.front_image_Queue.put)
        #time.sleep(30)
        #raise
        #print("Hello")
        #time.sleep(30)
        # with CarlaSyncMode(self.world.world, *list(self.cameras.values()), fps=20) as sync_mode:
        #     self.val = sync_mode.tick(1.0)
        #     print(self.val)
        
        try:
            self.sync_mode.__enter__()  # Explicitly enter the context
            self.snapshot, self.eyelevel_rgb, self.eyelevel_ins, self.eyelevel_dep = self.sync_mode.tick(2.0)
        finally:
        ...
        #with CarlaSyncMode(self.world.world, *list(self.cameras.values()), fps=20) as :
            #snapshot, eyelevel_rgb, eyelevel_ins, eyelevel_dep = self.sync_mode.tick(1.0)
            # image = util.get_image(eyelevel_rgb)
            # image_path = '_out/%08d.jpg' % image.frame
            # image.save_to_disk(image_path)

        
        #eyelevel_rgb_array = np.uint8(np.array(self.eyelevel_rgb_array))
        # self.image = self.eyelevel_rgb_array.reshape((self.im_height, self.im_width, -1))
        # list_of_files = glob.glob('_out/*') # * means all if need specific format then *.csv
        # latest_file = max(list_of_files, key=os.path.getctime)
        # print(latest_file)
        # loaded_image = cv2.imread(latest_file)
        # loaded_image = np.uint8(np.array(loaded_image))
        # loaded_image = cv2.resize(loaded_image,(640,360))
        # loaded_image = loaded_image[:, :, :3]
       
        #self.actor_list.extend([cameras['eyelevel_rgb']])
        print("Inside Setup", self.controller._control)
        #self.collision_hist.append(self.world.collision_sensor.get_collision_history())
        #self.actor_list.append(self.world.collision_sensor)
        self.info = dict()
        ...
        # self.eyelevel_rgb_array = util.get_image(eyelevel_rgb)
        # self.eyelevel_rgb_array = np.uint8(np.array(self.eyelevel_rgb_array))
        # self.eyelevel_rgb_array = cv2.resize(self.eyelevel_rgb_array,(640,360))
        # self.eyelevel_rgb_array = self.eyelevel_rgb_array[:, :, :3]
        # im = Image.fromarray(loaded_image)
        # im.save("your_file.jpeg")
        # cv2.imshow("walker camera View", loaded_image)
        # cv2.waitKey(1)
        # time.sleep(30)
        # print(loaded_image.shape)
        #self.world.tick(self.clock)
        
        #print(len(self.front_image_Queues))
        #print((self.front_image_Queue).qsize())
        # while self.front_image_Queues[1].empty():
        ...
        # image = self.front_image_Queue[1].get()
        # image = np.array(image.raw_data)
        # self.image = self.eyelevel_rgb_array.reshape((self.im_height, self.im_width, -1))
        # self.image = self.image[:, :, :3]
        # print(self.image.shape)
        #image_path = '_out/%08d.jpg' % self.eyelevel_rgb_array.frame
        cv2.imwrite('image.jpg',self.eyelevel_rgb_array)
        #self.eyelevel_rgb_array.save_to_disk('_out/image.jpg')

        return self.eyelevel_rgb_array,self.info
    ...
    def _step(self, action):#0- Local Policy #1- Global Policy
        ...
        # with CarlaSyncMode(self.world.world, *list(self.cameras.values()), fps=20) as self.sync_mode:
        #     snapshot, eyelevel_rgb, eyelevel_ins, eyelevel_dep = self.sync_mode.tick(1.0)
        #self.eyelevel_rgb_array = util.get_image(eyelevel_rgb)
        #print("Hi")
        # self.eyelevel_rgb_array = np.uint8(np.array(self.eyelevel_rgb_array))
        ...
        # loaded_image = loaded_image[:, :, :3]
        # #print(img.shape)
        # im1 = Image.fromarray(loaded_image)
        # im1.save("your_file_1.jpeg")
        #try:
            #self.sync_mode.__enter__()  # Explicitly enter the context
        self.snapshot, self.eyelevel_rgb, self.eyelevel_ins, self.eyelevel_dep = self.sync_mode.tick(2.0)
        #finally:
        ...
        if action==1:
            ...
            # self.world.player.set_location(goal_x,goal_y)
        ...
        #new_dist_from_start = Apply for on campus jobs for Fall-instalocationn)
        #square_dist_diff = new_dist_from_start ** 2 - self.dist_from_start ** 2
        #self.dist_from_start = new_dist_from_start

        
        count=self._get_num_people(self.eyelevel_rgb_array,self.model_yolo)
        ...
        # if (len(self.collision_hist)-1) != 0:
            print("Collision History",len(self.world.collision_sensor.get_collision_history()))
        ...
        if done or truncated:
            ...
            print("Task Metrics:")
            print("Collision Count:",len(self.world.collision_sensor.get_collision_history()))
            print("Route Completion",(self.count_route_steps/len(self.minimap.planner.path_pts)))
            self.task_metrics.append([self.count_epi,len(self.world.collision_sensor.get_collision_history()),(self.count_route_steps/len(self.minimap.planner.path_pts))])
            #print("Energy",)
            #self.collision_count+=1
            
        
        return self.eyelevel_rgb_array, reward, done, truncated,self.info
    ...
    def _run_global_policy(self):
        rotation = math.atan2( self.minimap.player_pos.y-self.minimap.goal_rect.y, self.minimap.player_pos.x-self.minimap.goal_rect.x)
