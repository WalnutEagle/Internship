...
class CustomValueCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        values = self.model.policy.predict_values(torch.Tensor(x).to('cuda:0'))
...
def learning_step(args):
    ...
    try:
        ...
        env = gym.make('carla-v0-eval-spinn')
        env = Monitor(env, log_dir)
        #print(type(env.action_space))
        #print(env.action_space)
        env.reset()
        policy_kwargs = dict(net_arch=dict(pi=[16, 16], vf=[256, 256]))
        model = PPO("MlpPolicy",env, policy_kwargs=policy_kwargs, verbose=1, device=0)
